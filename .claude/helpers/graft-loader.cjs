...
function windowsPathsTrusted(files, root) {
  ...
  try {
    ...
    const result = JSON.parse(execFileSync(shell, [
      '-NoLogo', '-NoProfile', '-NonInteractive', '-EncodedCommand', Buffer.from(windowsAclScript, 'utf16le').toString('base64'),
    ], { input: JSON.stringify([...queried.keys()]), encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'], timeout: 5000, windowsHide: true }));
    ...
    const trusted = new Set([
      result.user, 'S-1-5-18', 'S-1-5-32-544',
    ...
    for (const row of result.rows) {
      ...
      for (const rule of row.rules) {
        if (typeof rule.sid !== 'string' || typeof rule.allow !== 'boolean' || typeof rule.inheritOnly !== 'boolean' || !Number.isInteger(rule.rights) || rule.rights < -2147483648 || rule.rights > 4294967295) return false;
        if (rule.allow && !rule.inheritOnly && !trusted.has(rule.sid) && (rule.rights & mutationMask) !== 0) return false;
      }
...
function resolveEntry(name) {
  ...
  for (const directory of (process.env.PATH || '').split(path.delimiter)) {
    ...
    try {
      ...
      const cli = windows ? fs.realpathSync(path.join(directory, 'graft.cmd')) : trustedPath(path.join(directory, 'graft'));
      if (!cli) continue;
      const root = windows
...
module.exports = { resolveEntry, run, windowsPathsTrusted };
