...
export const hasFunctionStatic: {
  (schema: object, key: string): boolean;
  (key: string): (schema: object) => boolean;
} = dual(
  2,
  (schema: object, key: string): boolean => P.hasProperty(schema, key) && P.isFunction(Reflect.get(schema, key))
);
