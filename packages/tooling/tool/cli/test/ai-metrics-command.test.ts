    ...
    withTempDirectory((tmpDir) =>
      withRawArchiveKeyEnv(
        ...
        Effect.gen(function* () {
          ...
          yield* runAiMetricsCommand(["mirror", "build", "--target", "dankserver", "--data-root", dataRoot, "--json"]);
    ...
    withTempDirectory((tmpDir) =>
      withRawArchiveKeyEnv(
        ...
        Effect.gen(function* () {
          ...
          expect((yield* fs.readDirectory(parquetRoot)).filter((entry) => entry.startsWith("forwarder-"))).toHaveLength(
            1
          );
