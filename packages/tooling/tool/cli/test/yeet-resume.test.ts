...
const TestLayer = Layer.mergeAll(PlatformLayer, TestConsole.layer);
...
const options = (overrides: Partial<ResumeOptions> = {}) =>
  ResumeOptions.make({ ref: "42", list: false, print: true, force: false, json: false, agent: O.none(), ...overrides });

describe("yeet resume", () => {
  it.effect("parses number and URL references", () =>
    Effect.gen(function* () {
      expect(yield* parsePrRef("42")).toBe(42);
      expect(yield* parsePrRef("https://github.com/beep-effect/beep-effect/pull/43")).toBe(43);
    })
  );
