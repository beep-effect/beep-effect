      ...
      withTempWorkingDirectory(
        Effect.gen(function* () {
          ...
          const structuredIssueLine =
            ...
            '"severity":"warning","file":"packages/example/src/Example.ts","line":3,' +
            '"symbol":"WorkerError",' +
            '"message":"S.TaggedError declaration \\"WorkerError\\" declares a toEquivalence hook at the class. Effect derives a Schema class\'s equivalence from its declared field struct by construction (effect@4.0.0-rc.113 and later), so the hook is redundant and hides the derived law. Remove it; a field that must not take part in identity declares an always-equal equivalence on its own schema (Defect from @beep/schema).",' +
      ...
      withTempWorkingDirectory(
        Effect.gen(function* () {
          yield* writeSchemaFirstSourceFixture([
            ...
            "export class Widget extends S.Class<Widget>()(",
            "  { widgetId: S.String },",
      ...
      withTempWorkingDirectory(
        Effect.gen(function* () {
          yield* writeSchemaFirstSourceFixture([
            ...
            "export class WorkerError extends S.TaggedError<WorkerError>()(",
            '  "WorkerError",',
            "  { cause: S.Unknown.annotate({ toEquivalence: () => () => true }), workerId: S.String },",
            '  { description: "Worker execution failed." }',
            ") {}",
            "",
          ]);
      ...
      withTempWorkingDirectory(
        Effect.gen(function* () {
          yield* writeSchemaFirstSourceFixture([
            ...
            '  "ExistingError",',
            "  { workerId: S.String },",
            '  { description: "Existing documented failure.", toEquivalence: () => sameError }',
            ") {}",
            "export class WorkerError extends S.TaggedError<WorkerError>()(",
            '  "WorkerError",',
            "  { workerId: S.String },",
            '  { description: "Worker execution failed.", toEquivalence: () => sameError }',
            ") {}",
            "",
          ]);
