...
// Real git, run through the Effect spawner so the suite loads on Node and Bun alike.
...
describe("research daily commit", () => {
  ...
  it.effect("fails with the check-ignore exit code when the vault is not a git repository", () =>
    provideTestLayer(
      Effect.gen(function* () {
        ...
        const result = yield* Effect.result(commitVault(notARepo));

        expect(Result.isFailure(result)).toBe(true);
        if (Result.isFailure(result)) {
          expect(result.failure.message).toContain(`git check-ignore -q ${VAULT_DIRS.state} exited with 128`);
        }
      })
    )
  );
...
describe("research daily cognify gate", () => {
    ...
    provideTestLayer(
      Effect.gen(function* () {
        ...
        const result = yield* Effect.result(
          cognifyImpl(ResearchCognifyOptions.make({ dryRun: false, vaultRoot }))
        ).pipe(withConfig({}));

        expect(Result.isFailure(result)).toBe(true);
        if (Result.isFailure(result)) {
          expect(result.failure.message).toBe(COGNEE_CREDENTIALS_MISSING);
        }
      })
    )
  );
  ...
  it.effect("names the Cognee URL when the login request cannot connect", () =>
    provideTestLayer(
      Effect.gen(function* () {
        ...
        const result = yield* Effect.result(
          cognifyImpl(ResearchCognifyOptions.make({ dryRun: false, vaultRoot }))
        ).pipe(withConfig({ COGNEE_API_URL: UNREACHABLE_COGNEE_URL }));

        expect(Result.isFailure(result)).toBe(true);
        if (Result.isFailure(result)) {
          expect(
            Str.startsWith(`Cognee login request to ${UNREACHABLE_COGNEE_URL} failed: `)(result.failure.message)
          ).toBe(true);
        }
      })
    )
