...
> {
  ...
  const operation = Effect.scoped(
    Effect.gen(function* () {
      ...
      // The deadline race interrupts the capture and kills the child, but a child that handles the
      // signal by flushing a summary and exiting zero inside the kill grace can still deliver a
      // complete capture through the interrupted fiber (Lint Policy job 102722219371: Turbo exited 0
      // with 52 of 141 tasks after exactly the 15-minute cap and the step passed). A capture that
      // only ended once the deadline had elapsed is a timeout, whatever the child reported.
      if (!P.isUndefined(timeout) && Duration.isGreaterThanOrEqualTo(elapsed, Duration.fromInputUnsafe(timeout))) {
        return yield* CaptureCommandTimedOutError.make({
          commandLine,
          message: `${commandLine}: captured command exited ${exitCode} only after the ${Duration.toMillis(timeout)}ms deadline; tail: ${Str.slice(-2_000)(captured.text)}`,
        });
      }
