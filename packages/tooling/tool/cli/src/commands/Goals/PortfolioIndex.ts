/**
 * Generated goals portfolio index (`goals/INDEX.md`).
 *
 ...
 * `--check` fails on drift, following the repo's codegen-drift precedent
 * (docgen, laws --check). The file is never hand-edited; merge conflicts are
 * resolved by rerunning `--write`.
 *
...
).pipe(Command.withDescription("Generate goals/INDEX.md from packet manifests (--write) or verify it (--check)"));
