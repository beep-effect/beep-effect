...
const MISSING_ENTRY_REMEDIATIONS: Readonly<Record<string, string>> = {
  ...
  "SFV4-arbitrary-tests":
    "Add a focused property test using Arbitrary.schema(sourceSchema) and Arbitrary.checkEffect, or keep the inventory entry when the file is intentionally golden/snapshot/regression-only coverage.",
  "SFV4-fn-schema":
