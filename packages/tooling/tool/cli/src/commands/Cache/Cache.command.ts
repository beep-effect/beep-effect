...
const isWriteOnlyCacheSpec = (spec: string): boolean =>
  A.every(Str.split(spec, ","), (entry) => !O.exists(A.get(Str.split(entry, ":"), 1), Str.includes("r")));

/** Turbo defines `--force` as `--cache=local:w,remote:w`, so a read-less spec is a forced run. */
