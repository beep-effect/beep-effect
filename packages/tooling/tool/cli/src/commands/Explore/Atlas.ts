...
const ATLAS_LOG_PREFIX = "[explore:atlas]";
const ATLAS_WRITE_COMMAND = "bun run beep explore atlas --write";
...
export const renderExplorationAtlas: {
  ...
  let lines = [
    "# Exploration Atlas",
    "",
    `Generated by \`${ATLAS_WRITE_COMMAND}\`; this local file is not tracked GitHub truth.`,
...
// `CliReportedExit` is silent at the runtime boundary, so every refusal must
// print its evidence on stderr first or the operator sees only exit 1.
const refuseIssues = Effect.fn("Explore.refuseIssues")(function* (projection: ExplorationProjection) {
  const summary = `${A.length(projection.issues)} underivable projection input(s); repair the packet inputs, never ${EXPLORATION_ATLAS_PATH}`;
  yield* Console.error(`${ATLAS_LOG_PREFIX} ${summary}:`);
  for (const line of projectionIssueLines(projection)) {
...
const refuseDrift = Effect.fn("Explore.refuseDrift")(function* (drift: ReadonlyArray<string>) {
  ...
  yield* Console.error(`${ATLAS_LOG_PREFIX} ${summary}:`);
  for (const driftPath of drift) {
    yield* Console.error(`- ${driftPath}`);
  }
 ...
 * partial fleet projection can masquerade as current state. Every underivable
 * input is printed on stderr before the silent reported exit.
 *
 ...
 * @since 0.0.0
 */
export const checkExplorationAtlas = Effect.fn("Explore.checkExplorationAtlas")(function* (repoRoot?: string) {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const projection = yield* buildExplorationProjection(repoRoot);
  if (A.isReadonlyArrayNonEmpty(projection.issues)) return yield* refuseIssues(projection);
...
export const exploreAtlasCommand = Command.make(
  ...
  Effect.fn(function* ({ check, write }) {
    if (check && write) {
      yield* Console.error(`${ATLAS_LOG_PREFIX} --check and --write are exclusive.`);
      return yield* failWithReportedExit("explore atlas: --check and --write are exclusive.");
    }
    if (write) {
      yield* writeExplorationAtlas();
      yield* Console.log(`${ATLAS_LOG_PREFIX} wrote ${EXPLORATION_ATLAS_PATH} and README status projections.`);
      return;
    }
    if (check) {
      yield* checkExplorationAtlas();
      return;
    }
    yield* Console.log((yield* buildExplorationProjection()).atlasContent);
  })
