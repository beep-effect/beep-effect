...
import { renderSecurityPacketGuidance } from "./Security.packet.ts";
...
import type { CodexCaptureMethod } from "./Findings.triage.schemas.ts";
...
/**
 * Every phrase and document set that differs by capture source, in one table.
 *
 * Renderers index this by `plan.source` so a new source adds one row here
 * instead of another conditional in each document.
 */
const captureSourceCopy: Record<
  CodexCaptureSource,
  {
    readonly provider: string;
    readonly captureMethodPolicy: CodexCaptureMethod;
    readonly closeoutFixed: string;
    readonly stopCondition: string;
    readonly indexIntro: string;
    readonly indexVia: string;
    readonly closeoutLines: ReadonlyArray<string>;
    readonly guidanceDocuments: (plan: CodexPacketPlan) => ReadonlyArray<PacketDocument>;
  }
> = {
  "cloud-csv": {
  ...
  "security-bundle": {
    provider: "Codex Security CLI",
    captureMethodPolicy: "security-bundle",
    closeoutFixed: "packet-verified-fixed",
    stopCondition: "The sealed scan evidence cannot be verified.",
    indexIntro: "Captured from a sealed local Codex Security scan for",
    indexVia: "a digest-verified bundle",
    closeoutLines: [
      "- Record merged-revision fix verification in the packet ledger; never close local IDs in the cloud dashboard.",
      "- Record strictly proven invalid findings and their evidence in local triage.",
    ],
    guidanceDocuments: renderSecurityPacketGuidance,
  },
};
...
const manifestDocument = (plan: CodexPacketPlan): PacketDocument => {
  ...
  return PacketDocument.make({
    ...
    contents: json({
      ...
      lifecycle: "active",
      mission: `Remediate and close the ${capturedCount} Codex security findings captured on ${plan.capturedAt}.`,
      ...
      source: {
        ...
        captureMethodPolicy: copy.captureMethodPolicy,
      ...
      closeoutPolicy: {
        fixed: copy.closeoutFixed,
        alreadyFixed: copy.closeoutFixed,
...
const triageDocument = (plan: CodexPacketPlan): PacketDocument => {
  const ledger = CodexTriageLedger.make({
    meta: CodexTriageMeta.make({
      ...
      capturedAt: plan.capturedAt,
      captureMethod: captureSourceCopy[plan.source].captureMethodPolicy,
...
const goalDocument = (plan: CodexPacketPlan): PacketDocument => {
  ...
  return PacketDocument.make({
    ...
    contents: lines([
      ...
      `Outcome: fix and close the ${capturedCount} Codex security findings captured on`,
