 ...
 * **Details**
 *
 * The run's durable inner-lane report is read here, at step-failure time, so
 * each wrapper step's raw issue follows its first red recorded lane. A report
 * that cannot be read degrades to the whole-output fallback instead of masking
 * the step failure.
 *
