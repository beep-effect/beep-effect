...
// These 17 graduated cases retain the scratchpad core regression contract.

describe("fresh MemoryFileSystem test layers", () => {
  it.layer(MemoryFileSystem.layer)("first virtual volume", (it) => {
    it.effect(
      "writes to its virtual volume",
...
it.layer(MemoryFileSystem.layer)("FileSystem (memory-specific)", (it) => {
  describe("POSIX filesystem profile", () => {
    it.effect(
      "should resolve dot, dot-dot, and repeated separators",
      Effect.fnUntraced(function* () {
        const fs = yield* FileSystem.FileSystem;
        ...
        assert.strictEqual(yield* fs.realPath("/paths/child/../parent.txt"), "/paths/parent.txt");
        assert.strictEqual(yield* fs.realPath("/paths//child//file.txt"), "/paths/child/file.txt");
      })
    );

    it.effect(
      "should operate on a final symbolic link itself when removing or renaming it",
      Effect.fnUntraced(function* () {
        const fs = yield* FileSystem.FileSystem;
    ...
    it.effect(
      "should follow a final symbolic link for ordinary file operations",
      Effect.fnUntraced(function* () {
        const fs = yield* FileSystem.FileSystem;
    ...
    it.effect(
      "should fail when removing a missing path with force disabled",
      Effect.fnUntraced(function* () {
        const fs = yield* FileSystem.FileSystem;
        const error = yield* Effect.flip(fs.remove("/missing-without-force.txt", { force: false }));
        assert.strictEqual(error.reason._tag, "NotFound");
        if (error.reason._tag === "NotFound") {
          assert.strictEqual(error.reason.pathOrDescriptor, "/missing-without-force.txt");
        }
      })
    );

    it.effect(
      "should reject exclusive creation when a dangling symbolic link exists",
      Effect.fnUntraced(function* () {
        const fs = yield* FileSystem.FileSystem;
        yield* fs.symlink("missing.txt", "/dangling-link.txt");

        yield* Effect.flip(fs.writeFileString("/dangling-link.txt", "content", { flag: "wx" }));
    ...
    it.effect(
      "should leave both names unchanged when renaming one hard link over another",
      Effect.fnUntraced(function* () {
        const fs = yield* FileSystem.FileSystem;
    ...
    it.effect(
      "should report the source path when copy overwrite is disabled",
      Effect.fnUntraced(function* () {
        const fs = yield* FileSystem.FileSystem;
        yield* fs.writeFileString("/copy-source.txt", "source");
        yield* fs.writeFileString("/copy-destination.txt", "destination");

        const error = yield* Effect.flip(fs.copy("/copy-source.txt", "/copy-destination.txt", { overwrite: false }));
        assert.strictEqual(error.reason._tag, "AlreadyExists");
        if (error.reason._tag === "AlreadyExists") {
          assert.strictEqual(error.reason.pathOrDescriptor, "/copy-source.txt");
        }
  ...
  it.effect(
    "should serialize concurrent appends and exclusive creates",
    Effect.fnUntraced(function* () {
      const fs = yield* FileSystem.FileSystem;
      ...
      yield* Effect.all([first.writeAll(encoder.encode("A")), second.writeAll(encoder.encode("B"))], {
      ...
      assert.strictEqual(A.filter(creates, Result.isSuccess).length, 1);
      assert.strictEqual(A.filter(creates, Result.isFailure).length, 1);
    })
  );

  it.effect(
    "should leave bytes unchanged when file-size mutations are invalid",
    Effect.fnUntraced(function* () {
      const fs = yield* FileSystem.FileSystem;
      ...
      for (const size of [-1, 1.5, Number.MAX_SAFE_INTEGER + 1]) {
        const error = yield* Effect.flip(file.truncate(size));
  ...
  it.effect(
    "should store POSIX metadata without enforcing a virtual user identity",
    Effect.fnUntraced(function* () {
      const fs = yield* FileSystem.FileSystem;
      ...
      assertSome(info.uid, 42);
      assertSome(info.gid, 84);
      assertSome(
        O.map(info.atime, (date) => date.getTime()),
        1_000
      );
      assertSome(
        O.map(info.mtime, (date) => date.getTime()),
        2_000
      );
  ...
  it.effect(
    "should not traverse directory symbolic links while globbing",
    Effect.fnUntraced(function* () {
      const fs = yield* FileSystem.FileSystem;
      ...
      assert.deepStrictEqual(yield* fs.glob("**/*.ts", { root: "/glob" }), ["real/file.ts"]);
    })
  );

  it.effect(
    "should fail through the typed channel when copying a directory beyond the nesting bound",
    Effect.fnUntraced(function* () {
      const fs = yield* FileSystem.FileSystem;
      const nestedPath = `/bounded-source/${A.join(
        A.makeBy(258, (index) => `level-${index}`),
        "/"
      )}`;
      yield* fs.makeDirectory(nestedPath, { recursive: true });

      const error = yield* Effect.flip(fs.copy("/bounded-source", "/bounded-copy"));
  ...
  it.effect(
    "should publish committed normalized events for memory mutations",
    Effect.fnUntraced(function* () {
      const fs = yield* FileSystem.FileSystem;
  ...
  it.effect(
    "should publish updates through every current hard-link path",
    Effect.fnUntraced(function* () {
      const fs = yield* FileSystem.FileSystem;
      yield* fs.writeFileString("/original.txt", "content");
      yield* fs.link("/original.txt", "/alias.txt");

      const events = yield* watchEvents("/alias.txt", 1, fs.writeFileString("/original.txt", "updated"));
  ...
  it.effect(
    "should distinguish non-recursive and recursive directory watches",
    Effect.fnUntraced(function* () {
      const fs = yield* FileSystem.FileSystem;
      ...
      assert.deepStrictEqual(directEvents, [{ _tag: "Create", path: "/watch-tree/direct.txt" }]);
