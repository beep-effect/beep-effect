...
describe("AI Sync declared-field equivalence", () => {
  it("treats field-equal AiSyncError instances as equivalent and field-different ones as distinct", () => {
    ...
    const c = AiSyncError.make({ cause: O.some(new Error("sync failed")), message: "other failure" });
    const d = AiSyncError.make({ cause: O.some(new Error("other failure")), message: "validation failed" });

    expect(sameAiSyncError(a, b)).toBe(true);
    expect(sameAiSyncError(a, c)).toBe(false);
    // the defect cause is payload, never identity
    expect(sameAiSyncError(a, d)).toBe(true);
  });
});
