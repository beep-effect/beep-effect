...
const decodeDomainEnvVars = S.decodeEffect(Domain.EnvVars);
const isDomainAuthenticatedSnapshot = S.is(Domain.AuthenticatedSnapshot);
  ...
  layer(TestLayer, { timeout: "5 minutes" })((it) => {
    it.effect(
      ...
      Effect.fnUntraced(function* () {
        ...
        const envVars = yield* decodeDomainEnvVars({ NO_PROXY: "localhost" });
        ...
        expect(isDomainAuthenticatedSnapshot(snapshot)).toBe(true);
