 ...
 * onto the same canonical pair; an asymmetric relation never swaps.
 */
const orderEndpoints = (identity: LogicalEdgeIdentity): ReadonlyArray<string> =>
  pipe([encodeEdgeEndpointKey(identity.source), encodeEdgeEndpointKey(identity.target)], (endpoints) =>
    isSymmetricEdgeRelation(identity.relation) ? A.sort(endpoints, Order.String) : endpoints
  );
 ...
 * marker rather than being skipped — so no two distinct identities can produce
 * the same string by omission.
 *
 * @example
 * ```ts
