...
class PgliteHarness extends Service<
  PgliteHarness,
  {
    readonly migrationStatements: ReadonlyArray<string>;
    readonly noOpStatements: ReadonlyArray<string>;
  }
>()("@beep/effect-drizzle/test/integration/live.test/PgliteHarness") {}

class PgliteHarnessError extends TaggedError<PgliteHarnessError>("@beep/effect-drizzle/test/PgliteHarnessError")(
  "PgliteHarnessError",
...
const createOrganization = fn("EffectDrizzleLiveTest.createOrganization")(function* (suffix: string) {
...
const PgliteHarnessState = effectLayer(
  PgliteHarness,
  gen(function* () {
    const client = yield* PgliteClient;
    if (!(client.pglite instanceof PGlite)) {
      return yield* die("PgliteTestLayer did not expose a concrete PGlite client");
    }
...
const RepositoryClientLayer = unwrap(
  map(PgliteClient, (client) =>
    makePgliteLayer({
...
layer(PgliteHarnessLayer, { timeout: 90_000 })("@beep/effect-drizzle live PGlite gauntlet", (it) => {
  it.effect(
    ...
    fnUntraced(function* () {
      const { migrationStatements, noOpStatements } = yield* PgliteHarness;
      expect(isReadonlyArrayNonEmpty(migrationStatements)).toBe(true);
      expect(migrationStatements.some((statement) => statement.includes('CREATE TABLE "user"'))).toBe(true);
      ...
      expect(migrationStatements.some((statement) => statement.includes('CREATE TABLE "enum_export_collision"'))).toBe(
        true
      );
      expect(migrationStatements.some((statement) => statement.includes('"labels" text[]'))).toBe(true);
      expect(migrationStatements.some((statement) => statement.includes('"matrix" text[][]'))).toBe(true);
      expect(noOpStatements).toEqual([]);
    })
  );

  it.effect(
    "keeps deeper PostgreSQL expression restrictions database-checked",
    fnUntraced(function* () {
      const client = yield* PgliteClient;
      yield* flip(
        tryPromise(() =>
      ...
      yield* tryPromise(() => client.pglite.query("create table wave_e_partial_index (value integer)"));
      yield* flip(
        tryPromise(() =>
          client.pglite.query(`
  ...
  it.effect(
    "mirrors PostgreSQL value, default, action, and structural boundaries",
    fnUntraced(function* () {
      const client = yield* PgliteClient;
      ...
      yield* rejected("insert into wave_e_value_domains (small_value) values (32768)");
      yield* rejected("insert into wave_e_value_domains (integer_value) values (2147483648)");
      yield* rejected("insert into wave_e_value_domains (uuid_value) values ('not-a-uuid')");
      yield* rejected("insert into wave_e_value_domains (numeric_value) values ('not-a-number')");
      yield* rejected("insert into wave_e_value_domains (matrix) values (array[array['a'], array['b', 'c']])");

      yield* tryPromise(() => client.pglite.query("create table wave_e_action_parent (id integer primary key)"));
      yield* tryPromise(() =>
        client.pglite.query(`
      ...
      yield* tryPromise(() => client.pglite.query("delete from wave_e_action_parent where id = 1"));
      const actionRows = yield* tryPromise(() =>
        client.pglite.query<{ readonly parent_id: number | null }>("select parent_id from wave_e_action_child")
      );
  ...
  it.effect(
    "keeps char values faithful and enforces unique-column foreign keys",
    fnUntraced(function* () {
      ...
      const rejected = yield* flip(sql`insert into unique_source (target_id) values (${42})`);
  ...
  it.effect(
    "round-trips one- and two-dimensional arrays through the model repository",
    fnUntraced(function* () {
  ...
  it.effect(
    "executes the installed SqlModel repository and Option codec",
    fnUntraced(function* () {
  ...
  it.effect(
    "increments versions and rejects stale concurrent writers",
    fnUntraced(function* () {
  ...
  it.effect(
    "round-trips valid enums and surfaces invalid values as SqlError",
    fnUntraced(function* () {
  ...
  it.effect(
    ...
    fnUntraced(function* () {
      ...
      const root = getOrThrow(findFirst(queried.organizations, (row) => row.id === seeded.root.id));
      const child = getOrThrow(findFirst(queried.organizations, (row) => row.id === seeded.child.id));
      ...
      expect(member.organizationsThroughMembership.map((row) => row.id)).toContain(seeded.root.id);
    })
  );
});
