...
const decodeWithHostParser = Schema.decodeUnknownEffect(Schema.fromJsonString(Schema.Json));
...
const decodeNumberRecord = Schema.decodeUnknownEffect(Schema.Record(Schema.String, Schema.Finite));
...
const keyNeedsEscape = Effect.fnUntraced(function* (key: string): Effect.fn.Return<boolean, Schema.SchemaError> {
  return (yield* encodeJsonText(key)) !== `"${key}"`;
});

const hasKeyNeedingEscape = Effect.fnUntraced(function* (
  value: Schema.Json
): Effect.fn.Return<boolean, Schema.SchemaError> {
  ...
  if (isJsonArray(value)) {
    return yield* Effect.reduce(
      value,
      () => false,
      (found: boolean, child: Schema.Json) => (found ? Effect.succeed(true) : hasKeyNeedingEscape(child))
    );
  }
  return yield* Effect.reduce(
    R.toEntries(value),
    () => false,
    (found: boolean, entry: readonly [string, Schema.Json]) => {
      if (found) {
        return Effect.succeed(true);
      }
      const [key, child] = entry;
      return keyNeedsEscape(key).pipe(
        Effect.flatMap((needsEscape) => (needsEscape ? Effect.succeed(true) : hasKeyNeedingEscape(child)))
      );
    }
  );
});
...
it.effect.prop(
  ...
  Effect.fnUntraced(function* ([value]) {
    const text = yield* encodeJsonText(value);
    assert.equal(yield* encodeJsonText(yield* decodeJson(read(text))), text);
  }),
...
it.effect.prop(
  ...
  [JsonArbitrary],
  Effect.fnUntraced(function* ([value]) {
    assert.equal(AcpJson.hasEscapedPropertyKey(yield* encodeJsonText(value)), yield* hasKeyNeedingEscape(value));
  }),
  { arbitrary: fcRuns(200) }
);

it.effect(
  "keeps escaped one-character keys distinct from a materialised backslash key at the same position",
  Effect.fnUntraced(function* () {
    ...
    assert.deepEqual(yield* decodeWithHostParser('{"beep-acp-json":0,"\\\\":0}'), { "beep-acp-json": 0, "\\": 0 });
...
it.effect(
  "mirrors JSON.parse value semantics on the strict path",
  Effect.fnUntraced(function* () {
    const proto = yield* decodeJsonObject(read('{"__proto__":1,"a":1,"a":2,"\\u0041":true}'));
    ...
    assert.isTrue(Object.is((yield* decodeNumberRecord(read('{"\\b":-0}')))["\b"], -0));
    assert.deepEqual(read('{"\\f":[true,false,null]}'), { "\f": [true, false, null] });
  })
);

it.effect(
  "rejects texts outside the JSON grammar with the failing position",
  Effect.fnUntraced(function* () {
    ...
    for (const [text, position] of cases) {
      assert.equal(readFailure(text).position, position, `position for ${yield* encodeJsonText(text)}`);
    }
...
it.effect(
  "bounds nesting depth on the strict path",
  Effect.fnUntraced(function* () {
    const deepest = nestedArraysInsideEscapedKey(1023);
    assert.equal(yield* encodeJsonText(yield* decodeJson(read(deepest))), deepest);
    assert.equal(readFailure(nestedArraysInsideEscapedKey(1024)).reason, "Nesting depth exceeds 1024");
  })
);

it.effect(
  "fromJsonText round-trips a schema through JSON text and reports syntax failures as issues",
  Effect.fnUntraced(function* () {
    ...
    assert.deepEqual(yield* Schema.decodeEffect(codec)(encoded), value);
