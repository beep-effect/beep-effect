...
type PgErrorNameLiteral = keyof typeof PostgresErrorCodeByName;
type PgErrorCodeLiteral = (typeof PostgresErrorCodeByName)[PgErrorNameLiteral];

const PgErrorNameValues = Struct.keysNonEmpty(PostgresErrorCodeByName);
const PgErrorCodeValues = A.dedupe(A.map(PgErrorNameValues, (name) => PostgresErrorCodeByName[name]));
 ...
 * @since 0.0.0
 */
export type PgErrorName = typeof PgErrorName.Type;
 ...
 * @since 0.0.0
 */
export type PgErrorCode = typeof PgErrorCode.Type;
 ...
 * @since 0.0.0
 */
export const pgErrorEntries = A.map(PgErrorNameValues, (name) => [name, PostgresErrorCodeByName[name]] as const);

type PgErrorAliasesRecord = Readonly<Record<PgErrorCodeLiteral, A.NonEmptyReadonlyArray<PgErrorNameLiteral>>>;

const appendPgErrorAlias = (
  aliases: Partial<PgErrorAliasesRecord>,
  [name, code]: readonly [PgErrorNameLiteral, PgErrorCodeLiteral]
): Partial<PgErrorAliasesRecord> =>
  R.set(
    aliases as Readonly<Record<string, A.NonEmptyReadonlyArray<PgErrorNameLiteral>>>,
    code,
    O.match(O.fromUndefinedOr(aliases[code]), {
      onNone: (): A.NonEmptyReadonlyArray<PgErrorNameLiteral> => [name],
      onSome: (names): A.NonEmptyReadonlyArray<PgErrorNameLiteral> => A.append(names, name),
    })
  ) as Partial<PgErrorAliasesRecord>;
 ...
 * @since 0.0.0
 */
export const PgErrorAliasesByCode = A.reduce(
  pgErrorEntries,
  {} as Partial<PgErrorAliasesRecord>,
  appendPgErrorAlias
) as PgErrorAliasesRecord;
 ...
 * @since 0.0.0
 */
export const PgErrorCanonicalNameByCode = R.map(PgErrorAliasesByCode, (aliases) => aliases[0]) satisfies Readonly<
  Record<PgErrorCodeLiteral, PgErrorNameLiteral>
>;

const PgErrorCanonicalNameLookup: Readonly<Record<string, PgErrorNameLiteral | undefined>> = PgErrorCanonicalNameByCode;
const PgErrorAliasesLookup: Readonly<Record<string, A.NonEmptyReadonlyArray<PgErrorNameLiteral> | undefined>> =
