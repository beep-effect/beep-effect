...
| `McpCaller.ts` | `McpCallerIdentity`, `CurrentMcpCaller` | request-local caller identity populated by `sanitizedToolkit` dispatch; `clientId` identifies one **protocol exchange** (the HTTP protocol mints it per request), `sessionId` is the `mcp-session-id` header and the only stable per-session key — `None` for stdio, where the connection is the session |
