...
const decodeEgressDeniedSync = S.decodeSync(EgressDenied);
const encodeEgressDeniedSync = S.encodeSync(EgressDenied);
...
describe("EgressDenied", () => {
  it("constructs reason-free with only its tag", () => {
    ...
    // nothing else.
    const encoded = encodeEgressDeniedSync(denial);
    expect(Object.keys(encoded)).toEqual(["_tag"]);
  });

  it("round-trips through its schema", () => {
    const decoded = decodeEgressDeniedSync({ _tag: "EgressDenied" });
