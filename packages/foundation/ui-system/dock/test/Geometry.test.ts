...
describe("dock geometry projection", () => {
  it("partitions odd horizontal and vertical extents exactly at 50/50", () => {
    ...
    const horizontal = project(split("horizontal", 5_000, tabsOne, tabsTwo), { container: box, options });
    const vertical = project(split("vertical", 5_000, tabsOne, tabsTwo), { container: box, options });
  ...
  it("projects a nested three-way visual row and normalizes its shares", () => {
    ...
    const geometry = project(root, {
      container: DockBox.make({ left: 0, top: 0, width: 101, height: 20 }),
      options: GeometryOptions.make({ gap: 1 }),
    });
  ...
  it("keeps extreme and complementary ratios as exact partitions", () => {
    const assertRatio = (ratio: 1_000 | 9_000) => {
      const geometry = project(split("horizontal", ratio, tabsOne, tabsTwo), {
        container: box,
        options: GeometryOptions.make({ gap: 1 }),
      });
  ...
  it("allocates gaps and expands sash hit rectangles symmetrically", () => {
    const geometry = project(split("horizontal", 5_000, tabsOne, tabsTwo), {
      container: DockBox.make({ left: 10, top: 20, width: 101, height: 40 }),
      options: GeometryOptions.make({ gap: 3, minSashThickness: 9 }),
    });
  ...
  it("turns a zero-size container into finite zero-extent geometry", () => {
    const geometry = project(
      ...
      { container: DockBox.make({ left: 7, top: 9, width: 0, height: 0 }) }
    );
  ...
  it("projects an empty workspace to empty geometry", () => {
    expect(projectWorkspace(EmptyWorkspace.make(), { container: box })).toEqual(DockGeometry.empty);
  });

  it("supports data-last tree and workspace projection", () => {
    const node = split("horizontal", 5_000, tabsOne, tabsTwo);
    const workspace = PopulatedWorkspace.make({ root: node });
    const context = { container: box, options: GeometryOptions.make({ gap: 3 }) };

    expect(project(context)(node)).toEqual(project(node, context));
...
describe("minimum group extent clamp", () => {
  ...
  it("clamps the leading side up to the minimum with an exact partition", () => {
    const geometry = project(split("horizontal", 1_000, tabsOne, tabsTwo), { container: box, options });
  ...
  it("clamps the trailing side symmetrically", () => {
    const geometry = project(split("horizontal", 9_000, tabsOne, tabsTwo), { container: box, options });
    const [left, right] = geometry.groups;
  ...
  it("clamps on the vertical axis too", () => {
    const geometry = project(split("vertical", 1_000, tabsOne, tabsTwo), { container: box, options });
  ...
  it("degrades to the proportional partition when infeasible", () => {
    ...
    const geometry = project(split("horizontal", 1_000, tabsOne, tabsTwo), {
      container: box,
      options: infeasible,
    });
    const [left, right] = geometry.groups;
  ...
  it("defaults to zero and reproduces the unclamped projection exactly", () => {
    ...
    const implicit = project(node, { container: box, options: GeometryOptions.make({ gap: 3 }) });
    const explicit = project(node, {
      container: box,
      options: GeometryOptions.make({ gap: 3, minGroupExtent: 0 }),
    });
  ...
  it("leaves the hidden-sibling full-extent path unclamped", () => {
    ...
    const geometry = project(split("horizontal", 1_000, tabsOne, hidden), {
      container: box,
      options: clamped,
    });
...
describe("per-group minimum lookup", () => {
  ...
  it("sums leaf requirements through nested same-axis splits", () => {
    ...
    const options = GeometryOptions.make({ gap: 3, minGroupExtent: 30 });
    const geometry = project(nested, { container: box, options });
...
describe("per-panel constraints", () => {
  ...
  it("takes the maximum panel minimum in a group on each axis", () => {
    ...
    const horizontal = project(split("horizontal", 1_000, first, tabsTwo), {
      container: box,
      options: GeometryOptions.make({ gap: 3 }),
    });
    const vertical = project(split("vertical", 1_000, first, tabsTwo), {
      container: box,
      options: GeometryOptions.make({ gap: 3 }),
    });
  ...
  it("best-effort clamps split growth to panel maxima", () => {
    ...
    const geometry = project(split("horizontal", 9_000, first, tabsTwo), {
      container: box,
      options: GeometryOptions.make({ gap: 3 }),
    });
  ...
  it("lets minima win over conflicting maxima and stays total when all siblings are capped", () => {
    ...
    const geometry = project(split("horizontal", 5_000, first, second), {
      container: box,
      options: GeometryOptions.make({ gap: 3 }),
    });
