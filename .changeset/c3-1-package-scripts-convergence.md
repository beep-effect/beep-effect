...
Converge every workspace manifest's scripts block on the canonical package-scripts block: docgen moves behind beep:docgen with the manifest's existing text preserved, lint:deprecated-apis, lint:jsdoc and lint:laws are stamped per kind, doctest is stamped where in-source examples exist, codegen placeholders are removed, and the block is emitted in the canonical tier order. No runtime code changes.
