...
Every listed package's `tsconfig.check.json` now carries the references of its canonical tsconfig (written by `bun run beep tsconfig-sync --write`) and no longer overrides `module`/`moduleResolution`, so `check` resolves upstream packages through their declaration output instead of re-typechecking their source. Type-check configuration only; no runtime behaviour changes.
