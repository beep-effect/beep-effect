...
transcript no longer multiplies rows, and because the OTLP export drains on the
`ai_metrics_turns.otlp_exported_at_epoch_ms` watermark rather than on the ingest run,
it now exports only genuinely new turns instead of the whole store on every run.
