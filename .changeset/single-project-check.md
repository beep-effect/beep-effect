...
Type-check each package as a single non-emitting project (tsgo -p tsconfig.check.json with references cleared) so the check lane can never write another package's outputs; Turbo's check task now declares no outputs, scaffolds emit the check config, and the retired ts2306 torn-read flake class makes TS2306 a real signal again.
