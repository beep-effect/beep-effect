...
Pin the Effect catalog to the pkg.pr.new snapshot of main commit 330b7475e2
(2026-09-24, 111 commits after 4.0.0-rc.117) to get ahead of the rc.118
breaking changes, and migrate every package to its surface: the `effect/unstable/*`
modules move to the top level (`effect/unstable/httpapi` becomes `effect/http-api`,
`effect/unstable/arbitrary/Arbitrary` becomes `effect/Arbitrary`), `effect/Encoding`
splits into `effect/encoding/{Base64,Base64Url,Hex,EncodingError}`, and the Schema
range checks are renamed (`isLengthBetween` → `isBetweenLength`, `isSizeBetween` →
`isBetweenSize`, `isPropertiesLengthBetween` → `isBetweenProperties`, `isStartsWith` →
`isStartingWith`, `isEndsWith` → `isEndingWith`, `isIncludes` → `isIncluding`).
The platform-node-shared write patch is re-keyed to the snapshot URL, drizzle-orm
gains a declaration patch pointing its Effect adapter at `effect/sql/SqlError`, the
codegen-kit outputs are regenerated against the new paths, and the effect-vitest
detectors recognise the `effect/Arbitrary` provenance.
