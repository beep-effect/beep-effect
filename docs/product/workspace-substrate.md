...
- **Companion docs:** [Prose-to-Proof Vision](../PROSE_TO_PROOF_VISION.md) · [Prose-to-Proof PRD](./prose-to-proof.md) · [Dock kernel README](../../scratchpad/dockview/README.md) · [Living ledger (WHAT-IS-LEFT)](../../scratchpad/dockview/WHAT-IS-LEFT.md)
...
**The costs, honestly.** A vision doc that hides its costs is sales copy, so: the kernel diverges from dockview knowingly (binary ratio tree instead of N-child pixel branches, zipper activation instead of MRU, host-owned global focus, whole-record parameter replacement — each divergence and its price is written down in the README's [Divergences and their costs](../../scratchpad/dockview/README.md#divergences-and-their-costs)). And real capability gaps remain: popout windows (the genuinely hard iframe/webview reparenting problem), the deeper per-child constraint system (min/max, layout priority, snap-to-collapse), tab overflow, drop-indicator polish, context menus, a11y and keyboard docking, undo/redo consumers for the feed. [`WHAT-IS-LEFT.md`](../../scratchpad/dockview/WHAT-IS-LEFT.md) is the living ledger; when it and this document disagree, the ledger wins.
