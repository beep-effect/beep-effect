...
findings:
  ...
  - category: goal-critique
    confidence: high
    instruction: Keep an early closeout PR in draft until its local proof and final packet update are ready.
    explanation: PR 1038 merged five seconds after the last required hosted check passed, while local publication was still queued. All reviews were resolved, but hosted green did not prove local acceptance or include the final lifecycle update. The operator approved a successor draft PR to preserve that remaining work in one PR.
todos:
...
verifications cover all 108 affected owners. All thirteen review threads are
resolved. PR #1038 also merged before final closeout, while its final-head
local proof was still queued. The operator authorized successor PR #1042,
which is now open as a draft. The goal remains active until it contains the
final packet update
...
PR #1038 repeated that sequence despite its visible checklist. A draft is a
stronger safeguard: keep it draft through local proof and the packet update,
then verify the final head before marking it ready for the operator to merge.
Yeet currently exposes ready-only PR creation. For #1042, Yeet committed the
packet and passed its clean-HEAD installation. A manual early push and
`gh pr create --draft` then started remote review while that same full proof
continued. Keeping those steps in one draft-aware Yeet command would remove
the extra publication bookkeeping without weakening verification.
...
- Add a draft option to the early publication workflow so remote review can
  start without prematurely advertising the PR as ready.
