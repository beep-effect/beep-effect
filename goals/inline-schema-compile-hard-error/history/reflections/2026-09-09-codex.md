...
local proof was still queued. The operator authorized a successor draft PR.
The goal remains active until that successor contains the final packet update
...
Yeet currently exposes ready-only PR creation, so draft creation requires
the supported manual GitHub step after a verified push.
