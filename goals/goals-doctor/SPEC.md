...
| --- | --- | --- | --- | --- |
| Phase-status domain widened with `superseded` (D2 amendment) | `GoalPhaseStatus` | goals-doctor merge integration (2026-07-11) | PR #369 (merged to main mid-flight) marked gov-legal-data-driver-codegen's P2 phase `superseded` with a phase-level `supersededBy` — a token the 2026-07-11 census never observed. `superseded` is now a valid phase state, terminal for the packet but distinct from `complete`, so all-phases-complete checks stay honest. | None — permanent vocabulary; D2's normalization map is unchanged. |
