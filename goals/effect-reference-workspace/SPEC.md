...
One consolidated reference folder, `$HOME/YeeBois/references/effect/`, holds the upstream Effect
...
- Editing operator-owned `$HOME` files other than rendering the timer units, writing the status
  file, and creating `.repos/*` links (R14). `$HOME/.claude/rules/*` and memory files are follow-ups.
...
- Operator move (P1 S4, executed by the session, not a Codex lane): `$HOME/YeeBois/dev/effect` and
  `$HOME/YeeBois/dev/effect-tsgo` → `$HOME/YeeBois/references/effect/`; `git worktree repair`;
...
- [ ] `bash scripts/setup-effect-ref.sh <checkout>` is idempotent and produces `.repos/effect`,
      ...
      (default `$HOME/YeeBois/references/effect`).
...
- [ ] `$HOME/YeeBois/references/effect/graft/workspace.json` exists with children `effect`,
      `effect-tsgo`; `graft check $HOME/YeeBois/references/effect` passes; a federated
...
| Workspace index | `test -f $HOME/YeeBois/references/effect/graft/workspace.json && graft check $HOME/YeeBois/references/effect` | Passes |
...
| Upstream hygiene | `for m in effect effect-tsgo; do git -C $HOME/YeeBois/references/effect/$m status --porcelain; done` | Empty |
...
- `git worktree repair` cannot re-attach `$HOME/YeeBois/dev/effect-worktrees/docgen-enforce-examples`.
...
| File | Change |
| --- | --- |
| `$HOME/.claude/rules/effect-coding-standards.md` | Replace the `effect-smol` / `.repos/effect-v4` sentence with `.repos/effect` (effect `main` is v4) and mention `.repos/effect-workspace` for graft. |
| Claude memory pointers naming `$HOME/YeeBois/dev/effect` | Update to `$HOME/YeeBois/references/effect/effect` after the move. |
| `$HOME/.config/beep-graft/env` | None; reused as-is. If a separate env is ever wanted, copy it to `$HOME/.config/beep-refs/env` and re-run `install-timer --env-file`. |
