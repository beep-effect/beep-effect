...
One consolidated reference folder, `"$BEEP_REFERENCES_ROOT"/`, holds the upstream Effect
...
Operator path variables are bound and verified under PLAN.md “Operator path bindings”.
The manifest default and recorded R1 layout remain binding.
...
- Operator move (P1 S4, executed by the session, not a Codex lane): `"$OLD_EFFECT_ROOT"` and
  `"$OLD_EFFECT_TSGO_ROOT"` → `"$BEEP_REFERENCES_ROOT"/`; `git worktree repair`;
...
- [ ] `bash scripts/setup-effect-ref.sh <checkout>` is idempotent and produces `.repos/effect`,
      ...
      (default: the manifest's home-resolved `rootDefault`).
...
- [ ] `"$BEEP_REFERENCES_ROOT"/graft/workspace.json` exists with children `effect`,
      `effect-tsgo`; `graft check "$BEEP_REFERENCES_ROOT"` passes; a federated
...
| Workspace index | `test -f "$BEEP_REFERENCES_ROOT"/graft/workspace.json && graft check "$BEEP_REFERENCES_ROOT"` | Passes |
...
| Upstream hygiene | `for m in effect effect-tsgo; do git -C "$BEEP_REFERENCES_ROOT"/$m status --porcelain; done` | Empty |
...
- `git worktree repair` cannot re-attach `"$EFFECT_DOCGEN_WORKTREE"`.
...
| Claude memory pointers naming `"$OLD_EFFECT_ROOT"` | Update to `"$BEEP_REFERENCES_ROOT"/effect` after the move. |
