...
`src/index.ts` is intentionally curated rather than a barrel over internals. `src/pg/index.ts` is the future `@beep/effect-drizzle/pg` surface and exposes PostgreSQL combinators, `Table`, `schema`, and `toPgTable`.
