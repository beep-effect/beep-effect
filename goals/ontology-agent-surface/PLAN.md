...
Status: `active` — P2 is complete locally with socket and live-sidecar client proof host-gated; the inherited P0 ROBOT host gate remains open.
...
| P2 Transport + Safety | in-progress (host verification required) | Mount authenticated, Origin-validated streamable HTTP `/mcp` on the loopback sidecar; wire TierGate fail-closed and per-change actor attribution. Land the `capability-metadata` + `sparql-query` first slice as live proof before enabling mutation. | Local in-memory HTTP fallback proves the complete protocol and safety matrix; sandbox socket listen failed with `EPERM`. Run the recorded node-socket and live-sidecar MCP client commands on the host to complete the exit gate. Evidence: [`history/2026-07-11-p2-transport.md`](./history/2026-07-11-p2-transport.md). |
