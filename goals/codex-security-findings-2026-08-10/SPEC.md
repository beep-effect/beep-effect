# Codex Security Findings (2026-08-10) Spec

## Objective

Remediate every open Codex Cloud security finding visible at
`https://chatgpt.com/codex/cloud/security/findings/` for
`kriegcloud/beep-effect` in the 18-finding batch captured on 2026-08-10. Publish
...
- `goals/codex-security-findings-2026-08-10/**`.
- Gitignored raw evidence under `goals/codex-security-findings-2026-08-10/raw/**`.
...
- Browser closure happens after merge, against the exact 18-ID allowlist.
- Preserve unrelated work and stage only reviewed packet intent.

## Acceptance Criteria

- [ ] All 18 findings have sanitized tracked CSF records with Codex ID, severity,
...
- [ ] All 18 captured Codex findings are resolved after merge and the live view
...
| --- | --- | --- |
| Launcher size | `test "$(wc -m < goals/codex-security-findings-2026-08-10/GOAL.md)" -le 4000` | Pass |
| JSON shape | `jq .` over both files in `ops/` | Pass |
| Finding count | CSF file count equals 18 | Pass |
| Severity count | 4 High, 9 Medium, 4 Low, 1 Informational | Pass |
