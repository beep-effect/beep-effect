...
| D9 (2026-07-11) | **G1 accepted.** The concept and adapter matrices in [`research/architecture-proposal.md`](./research/architecture-proposal.md) route all product semantics to a new `landscape` slice (domain, use-cases, tables, server) or already-promoted vocabulary, and all transports to product-neutral drivers (`@beep/github`, `@beep/markdown-files`). The proof **declines** epistemic mechanism consumption, reusing only unconditional substrate (`TextAnchor`, `UnitInterval`, SHA-256 shape) and shared `ClaimLifecycle` vocabulary, so no cross-slice exception is authorized and the Exception Ledger remains `None`. Authority: snapshots, claims, evidence, provenance, lifecycles; briefs, observations, assessments, indexes, and views are rebuildable. |
| D10 (2026-07-11) | **G2 accepted.** [`research/technology-adr.md`](./research/technology-adr.md) selects the repo-native relational model with rebuildable projections for the first proof (criteria matrix 67/70 vs 44–45 for vendors). Cognee, Zep/Graphiti, TrustGraph, and mem0 are rejected as authority and as required P1–P3 runtime dependencies; they remain documented donors or later, separately gated projections. External license and behavior claims were web-verified on 2026-07-11. |
| D11 (2026-07-11) | **G3 accepted.** [`research/prototype-disposition.md`](./research/prototype-disposition.md) dispositions all nine `beep research` CLI tokens and every enumerated internal mechanism (including both systemd user-timer mechanisms) as promote/retire/defer with evidence and replacement ownership; no row is `reuse`. The three defer rows (Cognee client + cognify projection, Graphiti episodes, AI-metrics timer pattern) satisfy the D8 controlled-defer rules with proof of no P1–P3 dependency. |
| D12 (2026-07-11) | **G4 accepted.** [`research/architecture-proposal.md`](./research/architecture-proposal.md) fixes GitHub source identity to the provider repository database ID (locators become time-bounded aliases), content-addressed immutable snapshot envelopes with domain-separated derived IDs, explicit tombstone/purge state machines, terms-revision capture, claim support-lifecycle transitions on removal or license revocation, and deterministic post-deletion rebuild. Corpus verification observed a live provider-side rename, confirming the alias design ([`research/recon-report.md`](./research/recon-report.md)). |
| D13 (2026-07-11) | **G5 accepted.** [`research/architecture-proposal.md`](./research/architecture-proposal.md) places executable fixtures in a new package-local `github-landscape-loop` catalog under the landscape server `/test` surface (driver wire fixtures under the GitHub driver's `/test`), borrowing only the seed/input/expected pattern from `goals/agentic-professional-runtime`; packet copies are generated evidence only; consuming-slice tests stub boundaries and any real multi-slice composition proof lives at an app/integration boundary. |
| D14 (2026-07-11) | **G6 accepted.** Sources enter the first proof's watchlist through an explicit repo-relative, schema-versioned JSON watchlist file decoded at the server boundary into a typed `WatchlistSpec` (closed record; unknown providers/versions/duplicates rejected), with file hash recorded in run provenance ([`research/architecture-proposal.md`](./research/architecture-proposal.md)). Discovery, imports, and scheduled reconciliation remain roadmap growth over the same registration contract. |
| D15 (2026-07-11) | **G7 deferred-nonblocking.** The Obsidian-compatible vault and the MCP adapter (via `@beep/mcp-kit`) are projection-only candidates deferred until the canonical SDK is proven; the first proof ships one deterministic CommonMark daily brief, and no P1–P3 acceptance criterion depends on either surface ([`research/architecture-proposal.md`](./research/architecture-proposal.md)). |
| D16 (2026-07-11) | **D7 baseline discharged for P0.** [`research/threat-model.md`](./research/threat-model.md) adopts the `ingestion-security-secret-governance` exploration by citation, narrowed to the fixture-to-public-brief attack surface, with redact/quarantine/render-safe outcomes assigned per risk and an adversarial fixture-class catalog feeding P1; shipped `SafeRemoteHost` is reused (not rebuilt) for the future live adapter, whose transport hardening remains a roadmap obligation. |
| D17 (2026-07-11) | **First-source swap clause evaluated: declined.** Corpus reconnaissance shows GitHub as the dominant capture host and the highest-learning first source (identity-under-rename, license/attribution, artifact versioning all exercised); the GitHub watchlist stays the first source with the twelve verified seeds in [`research/interest-taxonomy-watchlist.md`](./research/interest-taxonomy-watchlist.md). |
