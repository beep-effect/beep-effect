...
- file:line: `packages/foundation/modeling/html/src/Html.conformance.ts:1791`
- symbol: `inspectElementOrder.description-list`
- members: `direct`, `wrapped`
- evidence: E1 at `Html.conformance.ts:1791-1818` — both values are derived
...
`significantText` rejection. Classification precedence is empty, direct,
wrapped, invalid; script-supporting-only remains empty and surrounding content
model checks retain responsibility for foreign content. Invalid retains the
parent path, `elementOrder`, and exact message `<dl> children must be complete dt+ / dd+ groups, directly or in <div> wrappers`.
Do not introduce stored state or a generic classifier.
...
- `Html.conformance.ts:1790-1821` — move the current wrapped predicate into the
...
- `Html.conformance.ts:2153-2169` — preserve content-model-before-element-order
  diagnostic ordering.
- `Html.conformance-hardening.test.ts:514-529` and
  `Html.coverage-matrix.test.ts:121-152` — migrate/extend exact empty, direct,
