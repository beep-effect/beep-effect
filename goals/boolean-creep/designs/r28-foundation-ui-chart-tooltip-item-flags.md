# Design: r28-foundation-ui-chart-tooltip-item-flags
...
`packages/foundation/ui-system/ui/src/components/chart.tsx:385–433` declares the
private React props owner `ChartTooltipItem`. It carries indicator: line | dot | dashed
...
full item/config; index; inherited formatter signature; optional color/nameKey; tooltipLabel: ReactNode; independent hideIndicator.

Content at520–532 is its only writer; Item forwards the pair only in the existing DefaultItem fallback at421–429. These private functions are not exported. Public ChartTooltipContent
...
Replace the pair with layout; use the Dot guard for alignment at417 and forward layout through the existing fallback. Preserve formatter execution and every Some result, including null, undefined and zero.
...
Remove its two correlated prop slots and paired fallback write. Replace the dot comparison with the derived Dot guard. The family removes the original nestLabel alias and compound dashed helper once, not once per record.
