...
- `packages/tooling/tool/cli/src/test/Docgen.test-kit.ts:32` and
  `src/test/Quality.test-kit.ts:79` — retain the test-kit export under the same
...
## R28 companion correction

`r3-tooling-jsdoc-migrate-rewrite-fence-gates` is archived as a covered
relationship in this migration, not an independent D1 or a separate refactor.
The frozen full companion proof is
`data/provisional-r28-jsdoc-migrate-fence-companion.md`.

| Previous marker present | Next marker present | Fenced | Existing line class |
| --- | --- | --- | --- |
...
These produced locals are 8/4; counting the incoming marker and its identical
wasOpen presence gives 16/4. This does not change the shared output's 4/3.
Both input and output markers retain the full String payload, including any
arbitrary or empty string accepted by the public dual input. Match the shared
three-way disposition and prior marker presence to produce the four existing
line classes. Remove the redundant wasOpen cache; retain the incoming marker.
All guard-deletion credit stays with this design.

Preserve all six downstream migration consumers: fenceBodies94–117,
tokenizeLinesMaskingFences121–131, splitBodyAndTags155–167,
splitTagSegments169–184, parseBodySections222–247 and
splitParagraphs249–267. Keep ordered masks, offsets, full source bytes,
content-conservation checks and quarantine outcomes. Exercise all four rows,
arbitrary markers and exact fence-byte conservation through existing tests.
This correction is native P2 evidence; no additional independent correction
or P3 approval is claimed. Implement in the ordered Tier 1E subsystem batch.
