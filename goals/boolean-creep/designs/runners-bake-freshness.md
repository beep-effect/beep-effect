...
- Retain qualified E4, stored/wire, tagged-union, Tier2,128/27. Fresh P2 status is designed; historical reviewed bytes remain archived and grant no replacement P3 credit.
- Private source-only proposal; no infra calls, package tests or implementation. Wider structural accepted-input narrowing remains an explicit P3 compatibility obligation.
...
The old design's “101 incoherent combinations” statement needs precision: 101 is the number of impossible binary presence/flag patterns, not the number of rejected concrete payload classes. Refining each actual into absent/equal/unequal gives `3^3 × 2^4 = 432` equality-aware strata, of which 27 are coherent and 405 contradictory. For example, an equal actual plus false match can fall inside a binary pattern that has a different, unequal coherent witness. the private `finite-projection.json` enumerates all 432 strata and the 27 coherent projections without claiming to execute Effect or the eventual implementation.
...
One named full-report transformation checks the old decoded Options, payload equalities, match flags, and overall flag at its boundary. If coherent, it constructs the three probe cases and retains `amiId`. Its encoder reconstructs all legacy fields from the cases, with `None` for missing actuals, `Some(actual)` for mismatches, `Some(value)` for matches, and a derived conjunction for `fresh`. Use `S.decodeTo` with installed `SchemaTransformation.transformEffect` and `SchemaIssue` failures for the fallible decode; encode is a total Effect projection for valid honest reports. Derive checks/guards from named schemas rather than duplicating hand-written type predicates. Keep the raw string schema and transformation at the boundary; do not round-trip through JSON in the service.
...
implementation; historical rc.112 line anchors are not current proof. No
package/lockfile change is needed. Apply annotations before `toTaggedUnion`
and preserve LiteralKit statics when composing annotations.
...
| Before d68 | Current source | Required migration or preservation |
| --- | --- | --- |
...
| `src/commands/Runners/Runners.service.ts:739-760` | `:740-751` | Keep first image, first matching tag, tag keys, AWS argv/region/pin and parse-error behavior; call the shared comparator as today. |
| `Runners.service.ts:762-785` | `:753-785` | Migrate the extracted comparator once. Keep raw SHA-to-Option decoding, version payload, and exact scalar comparison semantics; return the three probe results instead of duplicated Boolean locals/fields. |
| Absent | `Runners.service.ts:794-822` | Keep local-input load, manifest read/decode, YAML read/parse/schema decode, pin comparison, synthesized tags, and comparator call in that order. Do not bypass validations by directly fabricating a new report from manifest fields. |
| `Runners.service.ts:187-192` | `:189-194` | Both `check` and `checkManifest` return the honest Type under the same public report name. |
| `Runners.service.ts:830` | `:865-879` | Keep platform context capture and check/checkManifest provider bindings; no new requirements, layer, AWS effect, or scheduling change. |
...
The finite projection verifies cardinality algebra only. The required27 golden
rows must preserve full values, omission and non-semver observed version text
as a stale nonempty payload. They are not a substitute for byte comparison using the real old/new Effect codecs. At implementation, freeze the old schema/codec in test scope and compare its actual canonical string against the new codec on every row and schema-derived payload examples. Preserve the installed parser's default unknown-key behavior (`SchemaAST.ts:443-484`); no new excess-property rejection or defaults are introduced. Arbitrary input whitespace/property order was never preserved by parse/re-encode and is not promised as original-byte identity.

**Concrete accepted-input finding:** The current schema validates only field types. Four classes of structurally valid contradictory inputs must be covered: absent
actual with true match, equal actual with false match, unequal actual with true
match, and incorrect overall freshness. Their previous saved examples remain
historical evidence; this refresh does not claim to execute the decoder. Source inspection establishes that no coherence check rejects them; this is a static source conclusion, not a runtime test result. `JsonStringCodec.decode` and `decodeOption` at `internal/schema/JsonCodec.ts:79-85` inherit that permissiveness. A strict full-report transformation would reject them with `SchemaError`/`None`. Therefore, “all old accepted JSON inputs remain accepted” would be false.
...
2. Cover all four coherence-failure classes with explicit structural counterexamples for those four classes. Record old acceptance versus proposed rejection explicitly. These tests document the P3-approved boundary only after the accepted-legitimate-input question is resolved; they cannot themselves ratify narrowing.
...
This refresh corrects comparator/manifest anchors and the now-obsolete
transformation API. It does not count as a census, independent review, or
a ratification of a narrower public decoder contract. Source producers and
human/JSON consumers still support the coherent report domain; unsupported
contradictions cannot be resolved by merely observing no decoder callers.
...
The private audit binds immutable inventory/design inputs, current source,
fixtures, public facade, dependency declarations and installed/reference
transformation APIs. The broader decoder-acceptance question remains unapproved;
no implementation may silently assume its answer from this P2 refresh.
