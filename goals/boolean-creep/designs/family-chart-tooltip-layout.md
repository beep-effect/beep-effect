# Chart tooltip layout family

Current P2 shared migration for the three R28 ChartTooltip props cases.
Source `93217d998f851e2e93d9864e2b5315552eaa58a7`; main `d1b4d769fbaffddd55717f3b1ba461897dd545c5`.
The completed same-conversation UI correction confirms each 6/5 qualification
and the complete owner-level Boolean net. Independent P3 review is pending.
All three designs land atomically in Tier 1C and share this one private domain.
Source audit, provisional bytes and report hashes are retained in
`data/r28-chart-family-integration.json`.
...
The independent R28 correction confirms the distinction: each props owner has
two Boolean members for recall; its minimal correlated cluster includes the
co-carried literal and nesting Boolean. This does not admit one-Boolean owners.
...
- Keep story typechecking for the existing default, line and dashed public
  ...
  case; run the recorded browser-qa-loop for the affected interactive tooltip
  behavior, including record/extract/judge evidence with requiredCount: 0.
...
exported solely for tests. No product/package/browser commands were executed for this P2 design. The
implementation must supply the mandated package and recorded browser proof.

## Risk

The R28 correction confirms the owner-level two-Boolean net and minimal
6/5 relation for these three declarations. Independent P3 must still assess
taxonomy, full migration and compatibility. Companion migration scope does
not create additional qualified owners.
