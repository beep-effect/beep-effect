...
- `packages/foundation/ui-system/editor/package.json:50` — no edit: the existing `@beep/editor/chat/atoms` package subpath maps directly to the canonical owner; adding `TypeaheadMenu` to `atoms.ts` makes it package-alias accessible without a barrel change.
