...
- members: `write`, `check`
- evidence: E2 at `packages/tooling/tool/cli/src/commands/Fallow/Fallow.command.ts:481-493` — after rejecting combined `write && check`, the reader dispatches write, check, or print and never handles combined-true as a mode. The conflict guard is supporting evidence, not E3.
...
- `Fallow.command.ts:477-500` — resolve `PrintWriteCheckMode`, delete boolean carry, and exhaustively dispatch print/write/check. Supply an `onConflict` Effect that first emits exactly `fallow boundaries: --write and --check are mutually exclusive.` and then fails with exactly `fallow boundaries: choose either --write or --check.` via `failWithReportedExit`.
