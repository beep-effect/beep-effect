# Design: r28-foundation-ui-chart-tooltip-indicator-flags
...
`packages/foundation/ui-system/ui/src/components/chart.tsx:293–310` declares the
private React props owner `ChartTooltipIndicator`. It carries indicator: line | dot | dashed
...
itemConfig: ChartConfig[string] | undefined; indicatorColor: string | undefined; independent hideIndicator: boolean.

DefaultItem at370–376 supplies the pair; Indicator forwards it to the mark at309. These private functions are not exported. Public ChartTooltipContent
...
Replace its two correlated props and mark write with layout. Preserve the configured icon before the hideIndicator check, then the mark with unchanged color and CSS variables.
...
Remove its indicator/nestLabel prop slots and paired mark write. The shared NestedDashed replacement deletes the old compound helper only once for this family.
