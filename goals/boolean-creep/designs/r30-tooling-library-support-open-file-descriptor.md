...
in `history/designs/2026-09-14-pre-resume-r30-tooling-library-support-open-file-descriptor.md`.
...
Private `OpenFileDescriptor` 265–277 stores required fd:FileDescriptor,
inode:Inode, position:Cursor and readable/writable/append Booleans 269–271.
`State.descriptors` 283–285 is the private HashMap of these records.
Cursor 258–263 is S.BigInt. The current seek implementation 2021–2049 accepts
signed bigint offsets but rejects a resulting position before zero with
BadArgument before changing either an open descriptor or the closed cursor.
Nonnegative bigint positions, including positions beyond EOF, remain supported.
These payloads introduce no Boolean axes or nullable states.

The sole allocation 1760–1779 copies three values from the distinct OpenMode
and initializes position 0. Reads and later mutations cannot change the access
triple: read 1893–1896, positional write 1940–1943, seek 2037–2040 and truncate 2081–2084
spread the descriptor and replace only position. Append writing retains the
map 1938–1939; close 1831–1845 removes its record; initial volume 3005 has an empty
map. No public factory accepts a descriptor or State bag. Only make/layer are
exported 3146/3180, returning the FileSystem service.
...
cursor unchanged. `Coverage.test.ts:395–422` and the shared conformance
618–659 directly witness these semantics. Read-write without append is also
...
The separate MemoryFile.closedPosition remains available for seek
after close, with the same before-zero failure and returned bigint position. Generic construction of private classes does not establish a
...
inode and the current Cursor representation and seek contract. No default for access, no Option, no classes
...
appropriate; preserve supported nonnegative bigint positions in updates and the current
BadArgument failure before zero. Do not restore the pre-upgrade negative-position
behavior or narrow the cursor to a number. Define the
kit before the class; keep flag-to-access derivation after the existing OpenFlag
definition 736 to avoid forward-initialization dependencies. No module split or
...
| --- | --- |
|258–292|Keep the current Cursor, fd/inode schemas and HashMap/defaults. Define DescriptorAccess before the descriptor class; replace only its three stored access fields.|
|736–750,1760–1779|Joint seam with OpenMode 64/10 design: use the existing validated OpenFlag directly and collapse its ten values to five descriptor access cases. Preserve fd/counter/openCount/inode insertion and position 0. No opening-only flags survive in the descriptor.|
|762–781|Keep lookup-first closed failure, then requested permission failure, then File-inode existence/kind failure. Replace dynamic Boolean property lookup with derived access checks; preserve method and exact 'File descriptor is not readable/writable' descriptions.|
|1870–1915|Read and readAlloc still check readability before zero-length success. Nonempty reads retain position/size validation, copy only returned bytes, advance cursor by bytes read and update atime; zero-length IO retains cursor/state. Preserve untouched caller buffers on failures.|
|1918–1981|Write/writeAll still check writability before empty success. Append uses current inode byte length and does not advance stored cursor; positional writes use/advance cursor. Keep size checks, allocation-before-committed-byte-mutation, timestamps, buffer copying and nonzero-write events.|
|1831–1845,2001–2009|Closing removes the descriptor once, decrements openCount once, preserves inode until last open/link is gone and snapshots handle closedPosition under the same permit. Repeated close remains harmless.|
|2011–2049|stat/sync require an open File inode but no access capability. seek preserves signed offsets, start/current semantics, the returned bigint and handle-local closed cursor. A target before zero fails BadArgument before mutation for both open and closed handles. Spread/update access unchanged and never revive closed descriptors.|
|2055–2065|Keep readAlloc's number input, validation/allocation order, typed errors and Option<Uint8Array> result; no new optional descriptor state.|
|2067–2097|truncate retains its number input/default zero, writable check, allocation and timestamp/event updates. Clamp position only for nonappend access when greater than the new size. Appending cursors remain untouched even if beyond the truncation point.|
|2099–2112|write/writeAll/open signatures, number versus void results and acquireRelease finalization remain exact.|
|2222–2253|writeFile's temporary descriptor is allocated, written and closed within one volume mutation. Retain default w, complete bytes/mode inputs, error remapping to path/method writeFile and watch events.|
|2979–3034,3084–3111|Keep empty initial descriptor map, one-permit transactional state, cancellation/commit/event order and FileSystem.make method wiring.|
|src/MemoryFileSystem/index.ts:14; src/index.ts; package.json:19–20|Retain only make/layer facade, blocked implementation subpath and package root exports. No public descriptor schema, test-only export or new FileSystem API.|
...
Delete three stored Boolean fields 269–271 and three allocation copies 1765–1767;
...
Replace the dynamic Boolean lookup 772 with schema-derived readable/writable
membership. Replace append reads 1928/1938 and inverse append 2080 with one
...
permissions, arbitrary supported paths/bytes and large nonnegative bigint
positions. Preserve number read/write counts, number readAlloc/truncate inputs,
Option<Uint8Array>, void writeAll results and ByteSize file information. Keep
signed seek offsets whose resulting position is nonnegative, the bigint seek
result and before-zero BadArgument failure. Do not add a number-only cursor
refinement or reintroduce negative stored positions. Preserve descriptor IDs, inode identity,
...
The current API is Effect `4.0.0-rc.113`. Installed FileSystem.ts and the local
Effect reference at `51d4a2f08a5c7691dc876415bc9fc0ecf467e153` agree on the
read/write/readAlloc/seek declarations at lines 863–869; their whole files are
not identical. The source and current characterization fixtures are the
behavioral authority for the before-zero rejection.
...
Retain Coverage.test.ts 395–422 permissions/buffer/cursor checks,426–455 unlinked
handles,459–485 allocation failures, and Characterization.test.ts 127–152 before-zero
seek failure plus zero-length IO and 323–367 closed seek. Add zero-length forbidden
read/write assertions (permission check still precedes empty success), append
write with a valid nonzero read cursor, append truncate without clamping,
...
Shared FileSystemConformance.ts 579–702 supplies sequential read/write cursor,
...
subsets while preserving full fd/inode/bigint-cursor payloads. Restoring negative stored positions would
violate the upgraded fixtures; narrowing valid bigint positions to numbers would
also change the public contract. A 'closed' access variant would misrepresent
map absence. Both instances remain independently qualified, with shared-file edits
