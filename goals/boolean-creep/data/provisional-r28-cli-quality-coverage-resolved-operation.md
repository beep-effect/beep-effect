# Provisional R28 coverage resolved operation

- Proposed ID: `r28-cli-quality-coverage-resolved-operation`.
...
- Current owner: the successful returned object flow of `resolveCoverageTaskOptions`, anchored at `packages/tooling/tool/cli/src/commands/Quality/Tasks.ts:788`; kind `object-literal`; members `[replaceAll, writeBaseline, scoped, skip]`.
- Proposed classification: derived/internal, E1/E2/E4, representable 16 / legal 7; `LiteralKit` plus a schema-backed payload carrier; Tier 1.
- State: native provisional P2 design only. The independent R28 L–Q owner report and parent reconciliation are pending. This file admits no canonical row, changes no lifecycle status, and supplies no P3 acceptance or implementation authorization.

The source adjudication is `goals/boolean-creep/data/design-refresh-2026-09-09-r28-quality-scheduler-impact.md`, SHA-256 `5424d8dc0e0fe0f2e4f37f97cd71cf768f7c2bdb6e49e4c2d04c68bf6a2df9f9`. Its complete resolver/consumer proof is reused here; additional reads resolve the concrete schema placement, builder inputs, export exposure, and new observable testing return.

In this document, `Tasks.ts`, `Quality.schemas.ts`, `index.ts`, and `internal/...` are relative to `packages/tooling/tool/cli/src/commands/Quality/`. Test and test-facade paths are written relative to `packages/tooling/tool/cli/` when prefixed with `test/` or `src/test/`.
...
This proposal must supersede the overlapping `coverage-baseline-write-mode` qualification if independently admitted. The current design and its pre-main archive both have SHA-256 `e7999ecc6a3fba04eefd81f9815cdc4bd958db7f363facd51ce2263342f4a43d`; the archive is `goals/boolean-creep/history/designs/2026-09-09-pre-main-d1b4d7/coverage-baseline-write-mode.md`. Parent integration must archive/supersede the old current row/design together. Do not keep two qualified designs claiming the same resolved write/replacement state or guard deletions.

The related D records `quality-coverage-task-options` and `r26-cli-commands-l-q-coverage-replace-all-skip` require reconciliation and may retire as overlapping subsets when the full owner is admitted. Retain the raw `replaceAll/scoped` D1 contract under `r25-cli-commands-l-q-coverage-replace-all-scoped`, with the corrected explanation that raw parsing and pure planning support both true; successful runtime resolution does not. This design does not mutate any of those records.
...
E4 is the resolved implication `replaceAll ⇒ writeBaseline && !scoped`, established by `Tasks.ts:720/:725`. E1 is the sole skip-true producer at `:798–805`, which writes scope true after replacement was rejected. E2 is the ordered consumer policy at `:2991–3017`: report-only eligibility, noop exit, executor selection, then write/compare. These observations establish a state relation, not an assumption based on TypeScript representability.
...
Both noop variants must survive. `noop-ratchet` under `VITEST_COVERAGE_REPORT_ONLY=1` fails at `:2991–2994`; `noop-write` passes that policy check and exits at `:2997–2999` without cleaning, measuring, comparing, or writing. Collapsing them would alter behavior and the newly observable testing result.
...
All migrations below belong to one coordinated Tier 1 change, after independent admission, overlap reconciliation, and P3 acceptance.
...
| `Tasks.ts:2122–2135,2712–2714,2755` | Reduce `coverageStep` to the scalar/array inputs it consumes. Preserve raw planning for all write/replacement/scope spellings, baseline versus ratchet label, report-only env, exact Vitest suffix, command, cwd, step order, and both pure planner invocation forms. |
| `Tasks.ts:2137–2167,2764–2792` | Preserve full prebuild/shard construction, package filters, passthrough arguments, cache controls, file parallelism, worker caps and baseline report-only settings. Downstream Boolean parameters remain boundary projections. |
| `Tasks.ts:2811–2843` | Consume the resolved operation or case-narrowed payloads. Selected writes always use the existing shard path. Selected ratchets retain the existing nonempty-list and weight-threshold decision. The `scoped` flag check disappears only because dispatch already selects a scoped operation. |
| `Tasks.ts:2872–2904` | Preserve both dual forms and the anonymous `{ hosted, writeBaseline }` testing input exactly. Construct a scoped resolved case at `:2894`, keep package-filter normalization, and retain `usesShardedCoverageExecutor` arguments/behavior. Do not narrow this testing request or claim its function parameter is a census owner. |
| `Tasks.ts:2925–2967` | Full-step testing API, workspace coverage-owner discovery, lab exclusion, no-owners error, prebuild-first ordering, shard logs, and parallel runner/cap remain unchanged. |
| `Tasks.ts:2970–2984` | Dispatch selected write/ratchet cases with unchanged weighted/narrow behavior and distinct current baseline-regeneration versus weighted-ratchet log messages. Preserve single-step versus shard-step execution. |
| `Tasks.ts:2986–3017` | Apply the operation table above. Preserve report-only rejection before skip, no-work return before cleanup, cleanup before measurement, full/selected dispatch, and write/compare only after successful measurement. Project replacement true only for `unscoped-replace-all`. |
...
| `test/quality-tasks.test.ts:114,3080–3129,4361` | All indexed direct validator consumers. Existing failure assertions remain, exact-owner result access at `:3115` remains, and new operation expectations use the same test facade. Extend the existing resolver fixtures for all seven cases rather than creating an alternate mock resolver. |
| `test/quality-tasks.test.ts:2917–3134,4010–4093,4321–4371,4704,5141–5290` | Preserve pure planner behavior, cache/env settings, errors, exact filters, narrow baseline shards, hosted/local equivalence, affected planning and persisted baseline fixtures. Add only behavior-sensitive operation regressions. |
...
| `Tasks.ts:2819/:2832/:2841` | Replace the stored write read with case-derived shard input; eliminate the redundant scoped-state check after scoped dispatch; replace the write-or-wide selector with operation dispatch plus the existing workload predicate. Do not delete the nonempty/weight checks. |
| `Tasks.ts:2897–2900` | Remove the selected adapter's four-member object. Its one supported write input still chooses between the two scoped cases. |
| `Tasks.ts:2974/:2977` | Replace resolved write member reads with operation-derived scalar/log selection. Retain selected step-count and actual executor decisions. |
| `Tasks.ts:2991/:2997/:3003/:3009/:3011` | Remove all four member reads and their Boolean relation reconstruction. Exhaustive operation selection owns policy eligibility, noop, scope, baseline intent and the final replacement projection. The report-only policy/error, noop exit, execution and baseline side effects remain in the same order. |
...
Parent/independent admission must settle the actual return-object owner and all overlaps before replacing the current design. If admitted, use `r28-cli-quality-coverage-resolved-operation`; archive/supersede `coverage-baseline-write-mode` rather than landing both. Reconcile the two obsolete D subsets without removing the supported raw replacement/scope D1 contract. Preserve every R27 receipt and the pre-main design archive. Independent P3 review still must examine taxonomy, source reachability, raw-versus-resolved fidelity, export/test exposure, guard accounting and encoded preservation.

P2 validation is limited to this design: all eight required section strings, complete seven-row operation table, source-reference existence, frozen HEAD/main pins, source equality to HEAD, Markdown fence/whitespace checks, and unchanged prior audit/current design/archive hashes. Only `goals/boolean-creep/data/provisional-r28-cli-quality-coverage-resolved-operation.md` is written. No product source, tests, current design, canonical inventory, archive, earlier audit, dependency, status, service, Git index or ref is changed. No independent review result or qualified status is advanced.
