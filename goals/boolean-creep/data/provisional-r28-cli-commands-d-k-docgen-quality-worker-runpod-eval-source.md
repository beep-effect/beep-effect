# Provisional Runpod worker-eval source operation
...
replaced by this actual object. `requireRunpodEvalSource`929-942 rejects zero
or multiple sources; call1029 precedes nonnegative packet limit, positive
readiness timeout, template-search policy and worker/model/confirmation
validation1030-1033. Return the selection at1029 and construct the canonical
operation where the old object was built; preserve this order and all exact
errors. Source resolution still precedes GPU parsing and redacted API-key
configuration. Preserve provider/model/confirmRunpodEval, keepPod,
allow24GbFallback, gpuTypeIds Option/list conversion, templateId omission,
skipTemplateSearch, allowPublicTemplateSearch, readinessTimeoutMs, otlp flag,
otlpBaseUrl, otlpProject, all evaluator options1054-1074, scoped layer cleanup,
report generation1079 and optional output emission1080. This design changes
only the Runpod evaluator's source operation; it does not redesign those
separately owned authorization, template-search or cleanup states.
