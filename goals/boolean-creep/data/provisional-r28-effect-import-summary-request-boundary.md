# R28 replacement text: Effect Imports summary boundary

Native P2 data-only replacement for the dependent assumptions in
`designs/effect-import-rules-summary-operation.md`. Retains that stable Q4/3
and Tier2 singleton. Proposed D1 replacements for
`laws-effect-imports-command-options` and `laws-effect-import-rules-options`
must be adjudicated by the parent; their current designs remain untouched.
Frozen HEAD `93217d998f851e2e93d9864e2b5315552eaa58a7`; origin/main
...
- `Laws.command.ts:297-308,322-324`: retain current human rendering from raw
  options. Candidate stays operation=dry-run with candidate=true; the persist
  hint still depends on options.write. This no longer assumes a separate raw
  option migration.
...
Replace the current design's Target schema first paragraph, its option-owned
import/sequencing bullets, human-output ownership claim and final “Land after”
instruction with this independent summary boundary. Preserve its exact JSON
proof obligation. Archive the two raw-Q designs only if the parent admits the
proposed D1 corrections. This data addendum supplies no independent P3 credit
and no second bounded correction is implied.
