...
- evidence classes:
  - E4 — `packages/ontology/use-cases/src/aggregates/Session/Session.reasoner.ts:793-794`: the producer first derives `drifted`, then defines `fullRecompute = O.isNone(previous) || drifted`, proving `drifted => fullRecompute`. The pair is a flattened recompute-cause state.
...
- `packages/ontology/use-cases/src/aggregates/Session/Session.reasoner.ts:724-732` — change `recomputeMode` from its single `fullRecompute: boolean` input to the cause literal and derive whether it requires full work with the kit. This helper consumes only the implication's consequent today; it is not evidence that the result's two booleans are exhaustively branched together.
...
- `packages/ontology/use-cases/src/aggregates/Session/Session.reasoner.ts:724-731` — delete the boolean-first `if (fullRecompute)` interpretation of the derived consequent; the helper receives the cause domain directly.
