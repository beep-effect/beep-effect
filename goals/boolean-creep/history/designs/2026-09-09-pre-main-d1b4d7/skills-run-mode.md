...
- file: `packages/tooling/tool/cli/src/commands/Skills/Skills.command.ts:301`
...
All four CLI pairs retain behavior. Combined true fails with the exact
`SkillsCommandError` before `runSkillsUpdate` performs filesystem, repository,
or network work; the other three map to their existing literal values.

### 8. Test impact

`packages/tooling/tool/cli/test/skills-command.test.ts` directly calls the
workflow with `mode: "write"`; update construction if needed. Add command
coverage for all four raw pairs and the exact `SkillsCommandError` message.
Shared helper tests do not replace command-specific ordering and pre-work
conflict proof.
