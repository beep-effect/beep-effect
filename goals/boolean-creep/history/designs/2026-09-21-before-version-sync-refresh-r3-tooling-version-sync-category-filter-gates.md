## Six-category correction, 2026-09-21

Current source at `6db45e9d9182bc5d8f0fbae07b6fbe7727179559` adds `turbo`
and `turboOnly`. The pair remains 4/3. Preserve the ordered category domain
`bun`, `node`, `docker`, `biome`, `effect`, `turbo`; all 64 subsets are legitimate.
Each mode has seven independent flags including `skipNetwork`, with the same
false defaults. The category test matrix now contains 2,304 combinations across
subsets, queried categories, modes, and network settings.

Current owner references are `CategorySelectionService.ts:38–61`,
`VersionSync.schemas.ts:64,423–474`, `VersionSync.command.ts:82–108`, and
`ResolverService.ts:153–177`. The fixture at `version-sync-effect.test.ts:855–878`
includes the default order, turbo-only, and multiple selected categories.
Retain the new Turbo resolver and the refactored resolver warning, omission,
network and report behavior. The full resolver/updater contract audit remains
pending; older locators below retain their historical binding. This correction
supplies no independent-review or implementation credit.

# Current shape

R28 P2 design for stable `r3-tooling-version-sync-category-filter-gates`. Frozen source: HEAD `93217d998f851e2e93d9864e2b5315552eaa58a7`, origin/main `d1b4d769fbaffddd55717f3b1ba461897dd545c5`. The successful R28 R–Z/Yeet contract corrections independently confirmed source eligibility; this is designed, with independent P3 design review still pending. Full source and prior-design byte receipts are preserved by `data/r28-cli-last-integration.json`.
...
The raw request remains three `VersionSyncOptions` members (`VersionSync.schemas.ts:411–454`), each with six independent category Booleans and independent `skipNetwork`; all seven default to false on construction and decoding (`:406–409`). The mode vocabulary and six-category `VersionCategory` LiteralKit already exist (`:64–99`).
...
| `internal/services/ResolverService.ts:55,59,75,89,103,119` | Preserve all six category checks, resolver order, failures and conditional network requests. No signature migration. |
