...
The command adapter maps `(plan,json)` to the Option and retains the current error for `json && !plan`. Resolve it only after the existing `docgenConcurrencyConfig` read at `Docgen.command.ts:532`, preserving current error/effect precedence. Application code matches the Option once: `None` renders text and executes, `Some(text)` renders text and stops, and `Some(json)` renders JSON and stops. Both `Some` variants must retain the current `full-required` failure after rendering at `Local.ts:1313-1316`; plan-only does not always mean successful exit.
...
- `Local.ts:1297-1305` — `buildDocgenLocalPlan` accepts the migrated options shape but remains insensitive to output selection; preserve this direct public/test seam.
...
The application-layer guard disappears only because `planOutput` cannot
represent JSON execution. Bare CLI `--json` must still fail with the exact
current `DomainError` message before repository planning begins.
...
All four raw CLI pairs remain covered: three legitimate mappings retain their
observable behavior, while `json=true, plan=false` retains exact rejection.
...
executor while `None` executes. Retain the plan-only `full-required` cases and
their exact `docgen:local: full docgen proof required.` reported exit.
