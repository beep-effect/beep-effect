...
- evidence: E4 at `Files.service.ts:2244` and E1 at `Files.service.ts:1364`;
...
The service first computes effective dedupe at `Files.service.ts:2244`, then
calls the only plan builder at `:2256`. The private builder validates/resolves
the destination at `:1355` and calls the only options writer at `:1364` with
...
and implemented at `Files.service.ts:2244`. Preserve that D1 request carrier's
...
`Files.service.ts:2301`. Dispatch through the operation's schema-derived match
...
`Files.service.ts:2244` may remain as existing input-boundary behavior. This
design does not require rewriting the D1 request API just to remove those two
expressions. The profiling expression at `:2342` also preserves its current
...
| `Files.service.ts:332`, `:1364` | Replace the old Boolean/Option copying helper with construction of NormalizeManifestOptionsData from one handling variant and the unchanged format/maxLongEdge/overwrite fields. Do not route trusted constructors through an unnecessary legacy JSON round trip. |
| `Files.service.ts:1355`, `:1419` | Preserve directory validation and plan ordering; place the resolved destination only in the move payload. No new check rejects false/Some request input. |
| `Files.service.ts:2273`, `:2301` | Derive the logged destination from the move case and migrate the apply call to the plan-owned handling. Keep strings, dry-run, maxLongEdge, and overwrite behavior. |
...
| `test/files-command.test.ts:4210`, `:4267`, `:4307`, `:4368`, `:4412` | Migrate decoded manifest consumers; the dedupe/path assertions at `:4374`–`:4375` become handling assertions plus separate exact encoded-key assertions. Keep all existing behavioral assertions. |

`NormalizeFilesOptions`, `FilesCommandServiceShape.normalizeFiles` at
`Files.service.ts:275`, and exported `normalizeFiles` at `:2778` retain their
...
The existing-manifest refusal fixture at `test/files-command.test.ts:4483`
preserves its previous file contents on failure (`:4500`) and asserts no image
was written (`:4501`). Keep it. A refactor of options must not reorder preflight
...
| `NormalizePlan.duplicateDirectory`, schema `:263`, write `Files.service.ts:1420` | Deleted as redundant stored payload; plan, application, and manifest share the same move case. |
| Separate apply dedupe input, `Files.service.ts:2301`, `Apply.ts:455` | Deleted; the apply helper cannot be passed a Boolean disagreeing with its recorded options. |
...
| Raw-request folds at `Files.command.ts:759`, `Files.service.ts:2244`, and profile `:2342` | Retained behavior, not counted as deleted. They serve the supported D1 request/telemetry boundary. |
...
# Test and encoded-compatibility proof
...
4. Preserve the ordinary normalize fixture at `:4180`, dedupe-only at `:4278`,
   move-without-explicit-dedupe at `:4330`, dry-run non-mutation at `:4435`,
   overwrite behavior at `:4455`, and existing-manifest refusal at `:4483`.
   Assert decoded handling and exact legacy encoded fields separately.
