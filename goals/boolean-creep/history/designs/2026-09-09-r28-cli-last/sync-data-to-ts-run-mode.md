...
- `SyncDataToTs.command.ts:545-570` — preserve `findRepoRoot` at line 558
  before resolving flags at line 559, so root failure still precedes mode
  conflict. Preserve mode resolution before target-selection errors at line
  560. All later reads continue using the existing literal.
...
All four raw pairs retain their boundary behavior and exact typed conflict
message; existing report JSON continues to encode the resolved literal.

### 8. Test impact

Cover write, check, dry-run, and combined conflict through the command,
including exact message and root-before-mode/mode-before-target precedence.
Keep shared-helper unit coverage in `cli-kits.test.ts`; it does not replace
command-specific boundary proof.
