...
The operator's 2026-09-09 AGENTS instruction changes token-heavy Codex work to
`gpt-6-astra` with `xhigh` reasoning. Apply that setting to future native
design, implementation, exploration and review delegations; historical Sol
receipts keep their original provenance. The independent Grok census and
review venue specified by this packet remains separate from Codex delegation.
...
refresh. The live inventory is **674 records: 139 qualified and 535 disqualified** (D1
369 / D2 166), with **110 Tier 1 and 29 Tier 2**. Statuses are 23 historically
`reviewed`, 116 `designed`, and 0 `confirmed`; all need replacement P3 review.

Current source is `f03850b762e41217b5a0c26f26041daee490a070`, after merging
...
then 29 Tier 2 singleton PRs. Re-resolve and merge `origin/main` forward before every
review and publication operation; never rebase or merge a PR as the agent.

The current 110-record Tier 1 landing map is:
...
- **1E — internal tooling domains (46):**
  `package-verify-step-outcome`, `create-package-template-type-flags`,
  `runners-bake-cli-mode`, `tsconfig-sync-mode-flags`,
  `r2-tooling-bin-main-fast-paths`, `corpus-legacy-word-terminal`,
  `yeet-ack-resolution-flags`, `codegen-kit-cli-mode`, `worktree-removal-mode`,
  ...
  `r28-cli-internal-root-tmpfs-stub-observations`.

The 29 Tier 2 records are singleton PRs in this order:
