...
import { runStaticApiColocationCodemod } from "./static-api-colocation.codemod.ts";

const TestLayer = TSMorphServiceLive.pipe(Layer.provideMerge(NodeServices.layer));

const FIXTURE_DIR = "__fixtures__/static-api-colocation";
...
// Owner schema lives in another file and every candidate const is exported —
// nothing here should relocate.
...
export const isPublicGuard = S.is(ImportedThing);
...
const withSubject = <A>(
  ...
  Effect.gen(function* () {
    ...
    const dir = path.join(__dirname, `.tmp-static-api-${label}-${process.pid}-${Date.now()}`);
...
layer(TestLayer, { timeout: 300_000 })("static-api-colocation codemod", (it) => {
  describe("golden diff", () => {
    it.effect(
      "colocates same-file S.is/S.decode statics and rewrites local reads to Owner.method",
      Effect.fn(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
  ...
  describe("negative case", () => {
    it.effect(
      "leaves exported consts and imported-schema owners untouched (no write)",
      Effect.fn(function* () {
        const fs = yield* FileSystem.FileSystem;
