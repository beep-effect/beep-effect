...
- Prefer `S.Class` for object models and named intermediate schemas for reused
  concepts. The schema-first inventory flags any object schema built with
  `S.Struct` ("should prefer an annotated S.Class") unless a documented
  boundary exception applies. In a standalone fixture without `@beep/identity`,
  do not fall back to `S.Struct`. Use
  `class X extends S.Class<X>("X")({...}, { identifier, title, description }) {}`
  or an equivalent inline annotation, and do not add `@beep/identity` or other
  scaffolding imports. Exception: when the task names or describes a repo
  utility (for example "the repo utility that preserves the object shape"),
  import that exact helper, such as `import { O } from "@beep/utils"`. In a
  fixture task, "standalone" means the fixture's own configs must typecheck and
  lint without the parent repo. It does not forbid a repo helper that the task
  names.
...
- Treat broad exported/domain/boundary primitives such as `S.String`,
  ...
  be more precise.
- The schema-first precision audit (`SFV4-precision-audit`) is enforced.
  Any field whose name or meaning implies a format must not stay a bare
  `S.String`. Examples: email, url/uri, id/uuid, slug, phone,
  timestamp/date, currency/locale codes. This applies even when the field is
  wrapped in `optionalKey`, `NullOr`, `OptionFrom*`, or an array. Resolve it
  in one of these ways:
  1. Reuse the shared primitive, such as `@beep/schema` `Email`, when the
     fixture may import it.
  2. In a standalone fixture, define a small, named, annotated local schema
     and use it in every model that needs it. Examples:
     `export const Email = S.String.check(<pattern/format check>).annotate({ identifier, title, description })`
     plus `export type Email = typeof Email.Type`. Confirm the v4 check names
     in `Schema.ts`.
  3. Add a documented external-protocol exception comment when the wire
     format truly is arbitrary text.
  Apply this to every model in the file, including raw/wire input schemas.
  It also applies to fields the task did not mention.
...
- Run the scorer's checks from the fixture root against the fixture's own
    config, for example `tsgo`/`tsc -p .` and `biome check .`. Do not use a
    temporary config that you delete afterwards: the scorer only sees what you
    leave behind.
  - If `tsconfig.json` `extends` a relative path that does not exist in the
    copied location (TS5083), make the fixture tsconfig self-contained. Inline
    the needed options (`strict`, `module`/`moduleResolution`, `target`,
    `skipLibCheck`) and include a lib that provides `Disposable` and
    `AsyncDisposable` (e.g. `ESNext` or `esnext.disposable`). Effect's `.d.ts`
    files need these.
  - If Biome reports "No files were processed" or says the paths were ignored,
    the fixture is being excluded by parent VCS or include rules. Add a minimal
    fixture-local Biome config that does not inherit those ignores and
    includes `src/**`. Then re-run it and confirm it processes more than 0
    files.
