...
| P4 Ratchet PR (0.45.0) | pending | Bump tsgo; 13 ids at error; remaining config keys; confirm the roots S01 widened; regenerate inventories | `tsgo-rules` green at 0.45.0; hosted checks green; merge-ready; merged |
...
| S | scratchpad directives (78) + `vitest.shared.ts` (2) + widen the gate's scanned roots to `scratchpad` and the repo root | widened-root directive census returns only the allowlist |
| Z | tsgo 0.45.0 bump + 13 rules + keys + inventory regen (roots already widened by S) | `tsgo-rules` green at 0.45.0; all hosted checks |
