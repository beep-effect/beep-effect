...
const request = await Effect.runPromise(
  S.decodeUnknownEffect(CacheEntrypointReviewRequest)({
    ...
    artifacts: await Promise.all([
      artifact("cache-entrypoint-plans/v2", "entrypoint-plans-current.json"),
      artifact("cache-entrypoint-plans/v2", "entrypoint-plans-hosted-current.json"),
      artifact("cache-yeet-planner-review/v1", "yeet-plans-current.json"),
      artifact("cache-yeet-planner-review/v1", "yeet-plans-hosted-current.json"),
      artifact("cache-workflow-source-snapshot/v1", "workflow-sources.json"),
      artifact("cache-command-groups/v1", "command-groups-current.json"),
    ]),
...
await Bun.write(`${packet}/entrypoint-review-request.json`, `${text}\n`);
