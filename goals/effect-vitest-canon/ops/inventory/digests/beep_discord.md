# @beep/discord — P1 wave006-b digest
...
Layer topology and native boundaries: An effectful fixture allocates a capture Ref and supplies a fake HttpClient to Discord. Only one test sends the two sequential captured requests and resets the Ref first. No native/cloud HTTP service is acquired. Properties already use it.prop. There is no measured network or outer-layer startup cost.

Top files by actionable count (all assigned files; fewer than ten):

- packages/drivers/discord/test/Discord.service.test.ts: 1 actions, 4 rows; read 1–295.
- packages/drivers/discord/test/Discord.equivalence.test.ts: 0 actions, 4 rows; read 1–16.

Detailed findings:

- L-PROP-04, packages/drivers/discord/test/Discord.service.test.ts:291 (minor, confidence 1): The captured POST body is decoded and allowedMentions.parse is compared, but body.content is not compared with the submitted P1 proof string. A different schema-valid string can pass this capture oracle and the fixed response. Keep exact request URLs, authorization, response ids and allowedMentions, and assert the submitted content and POST method at the captured occurrence. Discord.service.ts:160-174 forwards request.content; this is missing distinguishing coverage, not a demonstrated driver defect.

Recorded runtime evidence: Root accepted one configured Node command baseline: 4 registered tests, {'passed': 4}; whole command 6.570897 seconds, reporter total 5843.792969 ms. These are different measurements. Slowest files (up to ten, including all represented files):

- packages/drivers/discord/test/Discord.service.test.ts: 37.792969 ms, 3 registered tests.
- packages/drivers/discord/test/Discord.equivalence.test.ts: 1.168701 ms, 1 registered tests.
