...
## 2026-09-25 — the A1 script double-counts lane time on post-A5 journals and misses every numbered `beep-effectN-worktrees` lane
...
- Evidence: since A5 a verdict lists the wrapper lanes and then the inner lanes each wrapper ran, and
  ...
  every duration-bearing lane in `lane_metrics` and in each episode's `laneDurationMs`, so on any
  journal written after A5 the same wall time counts twice there and the accounted share can pass
  100%. Its first-failure offset walk is not affected: it stops at the first failed lane that has a
  duration, and `buildYeetVerdict` lists every executed wrapper before any inner lane, so on the fleet
  it always stops at the failed wrapper. Its `discover_live_roots` globs `beep-effect[0-9]*` and
  `beep-effect-worktrees/*` only, while every numbered lane lives beside its clone as
  `beep-effect<N>-worktrees/<lane>` under the projects root (31 lanes with `.beep/yeet/runs` on
  2026-09-25); the first `yeet economics --fleet` read on 2026-09-25 found 87 checkouts with journals
  (its scope line lists 3 and `+84 more`). The A1 baseline predates A5 and is not affected.
- Prevention: the A3 surface splits the two populations with their own denominators (ruling 74) and
  ...
  script is fixed: it must split wrapper and inner lanes in `lane_metrics` and the episode lane
  minutes (by `parentLaneId`, else the ruling-74 prefix rule) and glob the numbered `-worktrees` roots
