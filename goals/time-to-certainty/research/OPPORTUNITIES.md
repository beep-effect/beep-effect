...
## 2026-09-04 — Merging A5b took eighteen hosted cycles for five real defects
...
## 2026-09-04 — Fallow's new-only duplication gate fires on every helper a lane writes twice
...
- **Evidence:** four consecutive pushes tripped Fallow on clone groups the lane had just created
  ...
  hosted cycle before the helper was extracted. The `normalizeJournal` split (cyclomatic 10) and the
  shared staging-file atomic rewrite are still owed after the merge.
