...
| `ci:fallow:health` (blocking since the census; revision 8) | ★ `fallow:health:check` | `… fallow health --check --base "$BEEP_PROOF_BASE" --out .beep/fallow/health.check.json --quiet` | as audit | D2; `env: ["BEEP_PROOF_BASE"]`; `outputs: [".beep/fallow/health.check.json", ".beep/fallow/raw/health.check.*"]` | C, G, W(`fallow:health`), Y | |
| `ci:fallow:boundaries`, `ci:fallow:flags`, `ci:fallow:security`, `ci:fallow:fix-preview` (advisory; health promoted to blocking after the census) | ★ `fallow:<lane>:advisory` (four root tasks) | `… fallow <lane> --advisory --base "$BEEP_PROOF_BASE" --out .beep/fallow/<lane>.advisory.json --quiet` | as audit | D2; `env: ["BEEP_PROOF_BASE"]`; `outputs: [".beep/fallow/<lane>.advisory.json", ".beep/fallow/raw/<lane>.advisory.*"]`; advisory exit semantics unchanged (the wrapper never fails the lane on advisory findings); the workflow gate and the post-run envelope checks (`check.yml:767–806`, `CiLane.ts:1235,1557`) stay | C, `check.yml:767–806` | advisory lane |
...
| `ci:repo-sanity` fallow boundaries (`repo-sanity:fallow-boundaries-config`) | ★ `fallow:boundaries:config-check` | `bun run beep quality fallow boundaries config-check --check` (`GithubChecks.ts:397`; distinct from the existing local root script `fallow:boundaries:check` = `beep fallow boundaries --check`, which stays a plain script and is the command the census section of that name describes) | `standards/fallow.boundaries.generated.jsonc`, `.fallowrc.jsonc`, `**/package.json`, `apps/**`, `packages/**`, `infra/**` (explanatory) | D2 (fallow binary walk) | C, W(`repo-sanity:fallow-boundaries-config`), G | |
