...
2. Session home resolution order, tagged in the record as `sessionHomeSource`: (a) `CODEX_COMPANION_TRANSCRIPT_PATH` if set, else glob `the Claude projects directory/*/<sessionId>.jsonl`, first record's `cwd` (`transcript`); (b) `the Claude session index/<CLAUDE_PID>.json` only when its `sessionId` equals the env id (`index`; also the sole source of `name`, `nameSource`, `entrypoint`); (c) checkout path (`checkout`). Codex: rollout header `cwd` from `the Codex session store/**` and `the Codex home/archived_sessions/**` matched by filename thread id, bounded 2 s, else checkout path; record `originator`. `findRecentClaudeSession` is deleted: with exact ids in the environment the mtime heuristic is only the documented wrong-session hazard, and with no id the honest answer is `unknown`.
...
3. Live guard (the common case: today every live index entry is `claude-desktop`): match `sessionId` against `the Claude session index/*.json` AND `/proc/<pid>` exists with matching `procStart`; Codex checks `the Codex cross-instance lock directory`. If live: print "live in desktop window <name> (workspace <label>, pid N) - open it there or SendMessage it" and stop; `--force` overrides. This alone answers the four-terminal hunt without spawning anything.
...
Why: The `cd` is redundant: `yeet resume` spawns the harness with cwd = the recorded session home, and any up-to-date checkout can run it. B and C are a path template plus a resume command, CSF-007's literal wording; they assert one level of clone structure, are functionally wrong for Codex worktrees under the Codex worktree root and for session-home != clone (scenario a), and the shim gives you the same one-paste ergonomics with zero path text in the PR. This decides whether BEEP_PROJECTS exists at all, so it comes first.
