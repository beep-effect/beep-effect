...
| --- | --- | --- | --- |
| Cross-slice promotion consultation | The new tenant-bound `@beep/shared-use-cases/PromotionGate` is law-neutral. The repository's only candidate-acceptance implementation derives its subjects from trusted fixture context and consults the gate; blocked verdicts become a typed refusal. Law-practice server supplies a fail-closed `CandorPolicy` adapter. No production acceptance composition root exists, so none is claimed. | Shipped contract and fixture-boundary behavior with focused shared, agent, and law-server tests; terminal no-live-composition finding. | terminal |
...
| Filing identity authority | Primary sources disprove a deterministic conversion: USPTO series codes span filing years while ST.13 requires a year. ST.13 now requires a known non-U.S. office code; exact representations remain distinct. A deployment guard refuses unresolved legacy ST.13 rows instead of letting exact JSON reads hide them. | Primary-authority-backed no-conversion boundary plus executable migration refusal; see `01-identity-authority.md`. | terminal |
...
specific: foundation capabilities may carry no product semantics, while the
promoted contract-only `shared/use-cases` package may own ultra-high-bar
cross-slice product ports. This packet follows the current law. The correction
is a routing update, not a deferral of the consultation.
