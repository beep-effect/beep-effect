...
| D-10 | **Acceptance-criteria revision (2026-07-30, first gauntlet run; supersedes the AC-2, AC-4, and AC-5 wording below).** (a) **AC-2 splits by layer.** Document and container rows must resolve through `kg_provenance` — met, and enforced strictly (no sampling). Graph nodes (family / application / patent) do **not** resolve in the shipped build; that is an unmet capability tracked as a blocker, not a passing criterion. A run satisfies the revised AC-2 by resolving every document row *and* explicitly marking every node-level claim unverifiable; a run that asserts spine facts as though backed fails. **AC-2 as a whole remains unmet until node provenance exists or is declared out of scope with a typed capability boundary.** (b) **AC-4 environment.** "In Claude Desktop on the workstation" is unachievable — Claude Desktop ships no Linux build and the workstation is Linux. The gate therefore runs in Claude Desktop on the Windows target with the packet operator driving, which is strictly closer to the delivered environment; AC-6 remains Tom installing and querying unassisted. (c) **AC-5 evidence standard.** "Under observation" is satisfied by interval sampling, which is what the first run performed; sampling cannot exclude a connection opening and closing inside one interval, so a sampled pass must be recorded as such. Enforced network isolation is the hard-proof upgrade and the standard for any claim stronger than "no connection observed". |
| D-9 | **Placement (architecture-guardian ruling, 2026-07-27).** An MCP host inherits the family of what it exposes; this one exposes law-practice slice language, so no `drivers/*` package and no new slice. Tool declarations → `law-practice/use-cases` (`.tools.ts`); handlers + KG projection writers → `law-practice/server` (`.tool-handlers.ts`, `.projections.ts`, `Tools.ts` composer); read-model tables → new `@beep/law-practice-tables` (scaffolded via `bun run beep architecture`); edge/node-kind literal domains → `law-practice/domain`; runnable stdio process + runtime layers + .mcpb + build entrypoint → new `apps/practice-kg-mcp`. The bundle build lane is NOT a repo-cli verb (tooling ceiling: repo-cli imports zero slice packages); contract §5 flags move to the app's build entrypoint. `law-practice/server` takes an injected SqlClient (no PGlite construction — DECISIONS 2026-07-08); the `@beep/epistemic-tables` import lands only with the claims work plus an Exception Ledger line and README record. Existing `drivers/*` MCP hosts are settled precedent (external engines), not drift. |
