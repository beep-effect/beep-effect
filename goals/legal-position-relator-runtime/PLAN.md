...
| P1 Rung 1: domain proof | in-progress | Schemas (`HohfeldPosition`, `LegalActContent`, both derivations, `Party`, `LegalRole`, `LegalPositionRelator`, `LegalScopeContext`) → the `LegalPositionRelatorPolicy` contract → failing-then-green `LegalPositionRelatorPolicy.test.ts` over in-memory layers. | All rung-1 acceptance criteria met; the test green after first failing; `packages/law-practice/domain` still declares no epistemic dependency. |
