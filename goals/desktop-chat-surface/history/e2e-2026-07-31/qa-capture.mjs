...

{
  ...
  const createButton = (await page.locator(tid("chat-no-thread-create")).count()) > 0 ? "chat-no-thread-create" : "sidebar-new";
...

{
  ...
  const userRendered = await page
    .waitForSelector("[data-testid='turn-user'], [data-testid='turn-streaming-user']", { state: "visible", timeout: 15000 })
    .then(() => true)
    .catch(() => false);
  ...
  assert(record, "assistant turn persisted in timeline", (await count("turn-assistant")) === 1, `assistant=${await count("turn-assistant")}`);
...

{
  ...
  assert(record, "cancelled turn persists only the stopped marker", assistants === 2 && lastAssistantText.includes("stopped"), `assistant=${assistants} last="${lastAssistantText.slice(0, 80)}"`);
  assert(
    record,
