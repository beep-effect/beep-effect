...
| Coverage Regression | fleet | fleet | 29.5m | Keep one fleet placement. Use exact directly changed coverage owners on PRs with an explicit full fallback; prebuild once and run four stable weighted in-job shards for complete runs. |
...
- Reject or roll back the four-shard admission if the existing 32-GB fleet
