...
| P2 Verify | active | `fallow:health:baseline:check` green; zero unwaived functions above cognitive 15; suppression total at most the latest-main comparator of 207 with no campaign additions; record 3 consecutive clean health-lane runs in `reports/clean-runs.md`. | Earlier full proof passed on the candidate based on `29f1284b43`. Main integration at `52fcc8d135` passed Oxlint, the affected package handoff, and three fresh health runs. Fresh full proof must cover the committed integration and semantic-delta documentation repair before push. |
