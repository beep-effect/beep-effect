...
{ "gateway": { "mode": "local", "port": 19013, "bind": "loopback" } }
EOF
env -i PATH="$NODEBIN:/usr/bin:/bin" HOME="$P/isohome" OPENCLAW_CONFIG_PATH="$S4/ctrl-config.json" OPENCLAW_STATE_DIR="$CTRL" \
  timeout 15 "$P/stage/openclaw-2026.7.1-2/node_modules/.bin/openclaw" gateway --port 19013 > "$S4/v2-ctrl-712.log" 2>&1
...
env -i PATH="$NODEBIN:/usr/bin:/bin" HOME="$P/isohome" OPENCLAW_CONFIG_PATH="$S4/ctrl-config.json" OPENCLAW_STATE_DIR="$CTRL" \
  timeout 15 "$P/stage/openclaw-2026.6.33/node_modules/.bin/openclaw" gateway --port 19013 > "$S4/v2-ctrl-633-back.log" 2>&1
rcx=$?
...
env -i PATH="$NODEBIN:/usr/bin:/bin" HOME="$P/isohome" OPENCLAW_CONFIG_PATH="$S4/future-config.json" OPENCLAW_STATE_DIR="$S4/future-state" \
  timeout 15 "$P/stage/openclaw-2026.7.1-2/node_modules/.bin/openclaw" gateway --port 19014 > "$S4/v2-future-guard.log" 2>&1
rcf=$?
...
echo; [ "$rc" = 0 ] && echo "SPIKE4-V2-PASS" || echo "SPIKE4-V2-FAIL"
exit "$rc"
