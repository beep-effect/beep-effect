...
| File | Role |
| --- | --- |
| `Models.command.ts` | `beep models` group: `check` (default), `catalog`, `init`; flags `--home <dir>` (default `$HOME` via `Config`), `--repo <dir>` (default: cwd checkout root), `--manifest <path>` (default `$HOME/.config/beep/models.yaml`), `--json`, `--report-dir <dir>`, `--offline` (skip the upstream fetch and assemble from the local overlays alone; an offline snapshot is never recorded as the ledger baseline). |
