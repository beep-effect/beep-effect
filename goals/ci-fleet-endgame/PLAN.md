...
| P2 Cutover | complete (#666) | Serve `beep-ec2-heavy` with ephemeral one-job-one-VM workers and demote manual burst scripts to break-glass. | Live worker passes lifecycle, teardown, trust, and red-team acceptance; heavy-lane wall time does not regress against the burst baseline. Landed 2026-08-11: 11/11 pickup gate, re-queue-to-fresh-runner proven live, burst registrations 75 → 6, PR waves ~20 min. Residue: the full guest-isolation red-team suite re-run on a live ephemeral worker stays open with the CSF-003 IMDS rework. |
| P3 Cache | active | Ship trusted-write/PR-read-only asymmetric Turbo cache access. | Real PR jobs consume cache without obtaining write authority. |
