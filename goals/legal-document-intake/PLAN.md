...
| P3 Box sync | complete (live-Box lanes deferred — see SPEC Exception Ledger) | One-way push sync process, durable sync state, remote-drift detection, Box OAuth setup UX. | Vault → Box mirror converges for create/move/rename; drift surfaces as conflict records; sync survives app restart via durable cursors. |
