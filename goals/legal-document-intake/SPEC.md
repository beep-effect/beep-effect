...
| D6 | KG storage | Postgres/PGlite projection: nodes/edges as schema-first tables; embeddings via pgvector; two-hop traversal via recursive SQL. Resolves the ontology-survey scope now absorbed by `semantic-foundation` in favor of projection; see `explorations/legal-ontology-landscape`. Dedicated graph DB stays a later optimization behind the same port. |
