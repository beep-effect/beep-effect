...
- **Verdicts are values:** fail-closed outcomes are `Denied`-style verdict values carrying
  audit records (`TierGateVerdict`/`ClaimGateResult` precedent). `TaggedErrorClass` errors
  only for real boundary failures (evidence decode failure, invariant violation). No new
  error kind crosses any boundary.
