...
`programmatic` activation paths are not user-visible; they satisfy the atlas
through `verified-source` evidence or the P1 resolver tests, and the
live-evidence gate counts only user-visible surfaces.
...
| Atlas artifact | `bun run goals/lexical-playground-capability-atlas/ops/verify-capability-atlas.ts` | `editor-capability-atlas/v1` decodes and files resolve; the entry gate requires every unverified capability to have an approved waiver, and the activation-path gate requires every user-visible unverified path to belong to a waived capability |
...
| user-approved 2026-08-24 — pinned upstream defect | `comments.threads` | `document-annotation-substrate` | The annotation candidate owns the pinned narrow-layout Comments defect under D11. | The candidate goal proves a reachable close control and Export with the comments panel open at 480 px. |
| user-approved 2026-08-24 — pinned upstream defect | `node.mark` | `document-annotation-substrate` | The annotation candidate owns the pinned marked-range interaction defect; the Lexical mark remains the editor projection under D11. | The candidate goal proves that marked-range paste does not trap the user. |
| user-approved 2026-08-24 — pinned upstream defect | `document.comments-panel` | `document-annotation-substrate` | The annotation candidate owns the pinned fixed-overlay and hidden-toggle defect under D11. | The candidate goal proves that the comments panel is not a fixed overlay at narrow widths. |
...
| user-approved 2026-08-24 — unavailable upstream | `table.column-reorder` | `configurable-full-document-editor` | The pinned table action menu at `packages/lexical-playground/src/plugins/TableActionMenuPlugin/index.tsx:467-670` has no reorder or move-column item. The only route is the hover `Drag to reorder column` control at `packages/lexical-playground/src/plugins/TableHoverActionsV2Plugin/index.tsx:626-665`, backed by `$moveTableColumn` at lines 500-528, so Goal B must generalize an accessible non-drag reorder route under D13. | Goal B records a keyboard or menu reorder route with evidence. |
| user-approved 2026-08-24 — unavailable upstream | `document.read-only` | `configurable-full-document-editor` | The pinned Playground has no mount-time read-only input (`packages/lexical-playground/src/App.tsx:326-350`; `packages/lexical-playground/src/appSettings.ts:14-44`). The lock calls `editor.setEditable` at runtime (`packages/lexical-playground/src/plugins/ActionsPlugin/index.tsx:354-361`), and the negative reload evidence at `goals/lexical-playground-capability-atlas/history/p0-exercise/2026-08-24/document.read-only/observations.ndjson:19-24` proves that it does not survive remount. | Goal B proves a read-only mount profile with all authoring commands disabled. |
| user-approved 2026-08-24 — product-host path | `interchange.canonical-json` | `lexical-playground-capability-atlas` | No Playground or current `@beep/editor` surface imports canonical `@beep/md` JSON; P1 builds the Storybook fixtures over the canonical document and P2 records that import. | P2 recorded browser QA proves the canonical JSON import lifecycle. |
