...
The negative dispatch is not free: each queued job costs one candidate launch
that idles for `minimum_running_time_in_minutes` before terminating. That is a
cost observation for the packet ledger, not an admission gap.
...
| 2. Registration reports the group; absence or rejection leaves the runner offline without fallback | pass | Fail-closed half: "Registration incident". Positive half: both probe candidates registered in group 4 and nowhere else (`17:48:17Z`, `17:48:47Z`). |
