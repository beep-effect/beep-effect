...
Status: **v1.0 — RATIFIED by Benjamin 2026-08-24 (MAP grill M1–M6 applied; see Dispositions).** Derived from `BRIEF.md`
...
| --- | --- | --- | --- | --- |
| `semantica-canary` | Scaffold the headless-first Tauri lab at apps/labs/semantica and run the staged canary C0 → C1 → C2 over F1 + W1 under the probe breaker (S1), emitting replay-identical `EvalReport`s; each passing stage flips its families from park-pending-canary to a verdict (B1). | `openai-driver` before C1 (not before C0) | Full table in the Capability Check below; NET-NEW is lab-local: `CanonicalText`, `ProviderCache`, ρdf `Reasoner`, EYE oracle wiring, `EvalReport` | **promised-now — Goal 1** |
...
| `ProviderCache` | C0 | content-addressed on-disk cache, key = `sha256(ModelIdentity ⊕ prompt/config hash ⊕ input hash)`, immutable, no TTL | — | **yes** (small; key schema is part of the shared schema, rabbit hole 5) | replay run with network off reproduces `EvalReport` bytes (G7); API-unavailable is a typed degraded state |
| `Evaluator` → `EvalReport` | C0 | schema-validated report: corpus hash, `gold/v1`, per-metric scores, Tier-L results, Tier-D telemetry; metric *names* from upstream #574 (T3) | `beep qa` inventory discipline (`qa-inventory/v1` in `packages/tooling/tool/cli/src/commands/Qa/JudgeCheck.ts`) | **yes** — lab-local `EvalReport`/`EvalRun` schemas with the `gold.proposer.provider !== extractor.provider` refinement (S2) | keep "how correct" split from "how fast" (#228/#231); no success-shaped dummy metrics (#1143) |
...
5. **Post-C2 (queued, reopen here at `decompose` when a gate fires):** `semantica-storage-inversion`
   ...
   needs the `G-entailment/rules` fixture. `semantica-atlas-sync` is eligible earlier (C0 pass or
   semantica 0.6.7+). The two O4 OSS gates sit behind the spike and behind a non-semantica reuse
   proof respectively.
...
- the two `EvalReport`s are byte-identical (replay identity, G7);
...
- the report carries corpus hash, `gold/v1`, `ModelIdentity` for every hosted call, Tier-L
  results (cold start, p95) and Tier-D telemetry.

Passing the slice does not pass C0; C0 completes when the same holds over F1 + all three
G-relation papers. The slice exists so that the first day of lab code hits a real paper.
...
the breaker's single retry, MuPDF parked (M1).
