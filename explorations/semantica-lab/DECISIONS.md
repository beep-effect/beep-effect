...
- **R2 (reasoning spike: the fixture is P1).** *Recommended:* amend the v1.0 precondition
  ...
  each) generated by extending `apps/labs/semantica/scripts/generate-g-entailment.ts` under the same EYE pins.
...
- **R2.c (twenty cases, five classes, oracle split).** *Q:* twenty cases, four per class (R-a
  ...
  `apps/labs/semantica/scripts/generate-g-entailment.ts` under the same EYE pins, with EYE gold separated from
...
- **R3.e (homes; "zero schema" means zero Notion schema).** *Q:* script home
  ...
  the exploration's `research/atlas/verdicts.json` under a NET-NEW `atlas-verdicts/v1`
