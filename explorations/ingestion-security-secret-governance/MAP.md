...
| GATED | Content | `safe-html-sanitization` | Establish pure safe HTML policy plus server and browser sanitizer adapters with an explicit trusted-output contract. | Browser-boundary spike must prove whether raw HTML or typed AST reaches the browser. | Reuse `packages/foundation/modeling/html/src/Html.meta.ts`, `Html.attributes.ts`, and `Html.model.ts`; extend with `SafeHtmlAttributes`, URL schemas, and markers. **NET-NEW:** safe subset and output contract; `sanitize-html` server adapter and DOMPurify browser adapter. |
...
| HTML metadata/AST | `packages/foundation/modeling/html/src/Html.meta.ts`, `packages/foundation/modeling/html/src/Html.attributes.ts`, and `packages/foundation/modeling/html/src/Html.model.ts` | **EXTEND** with a safe subset/marker. Live `GlobalAttributes` event handlers and `style` are explicitly unsafe. Runtime sanitizers are **NET-NEW adapters**. |
