...
1. Run `bun run beep explore atlas` for the local generated exploration map.
...
(align surfaces a research gap; shaping reopens a decision). Before stream
opt-in, the manifest stage is the explicit adoption snapshot, initializing
both D3 stage values. After `ops/events/` exists, `furthestStage` and
`resumeStage` derive only from the event fold. A later-stage artifact may be
pre-seeded when its content lands early; artifact presence is never state.
...
flowchart TD
    ...
    subgraph PACKET ["explorations/slug/ — state authority:<br/>manifest adoption before opt-in · event fold after"]
        direction TB
    ...
    ATLAS[["local D3 Atlas — generated by beep explore atlas"]]
    SCAFFOLD -.-> ATLAS
    GRAD -.-> ATLAS
    PARKED -.-> ATLAS
    KILLED -.-> ATLAS
...
| Status | Meaning |
| --- | --- |
| `active` | Being worked; appears in the generated Atlas active group. |
...
| `killed` | Explicitly rejected. Its one-line epitaph stays in tracked `DECISIONS.md` or README Trail; the generated Atlas records only state. Killing an idea is a successful outcome, not a failure. |

## Manifest Schema

`ops/manifest.json` is the tracked transition metadata the `/explore` skill
reads first. Before stream opt-in, its status and stage are the explicit
adoption snapshot. Once `ops/events/` exists, the PacketCore fold is
authoritative and the manifest fields are checked compatibility mirrors.
...
- Once all promised-now goals exist, flip the exploration status to
  `graduated`, update authored README prose, and run
  `bun run beep explore atlas --write`. Gated candidates remain in `MAP.md` as
...
- Every session that touches a packet ends by writing the next open question
  into the packet `README.md`, syncing the manifest, and running the Atlas
  writer — that is what makes cold-session resume instant. Never hand-edit the
  marked README Stage/Status region.
- `ATLAS.md` is an ignored local D3 projection, never doctrine or tracked
  GitHub truth. Load-bearing prose belongs in `docs/product/`, packet
  `DECISIONS.md`/Trail, or a goal packet.
