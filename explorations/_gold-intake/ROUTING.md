...
| P2 | MCP server design (conditional registration, multi-provider auth, progressive disclosure) | new-exploration | mcp-auth-gated-registration ✗ | mcp-auth-gated-registration | 28 | 11/12/5 | credential-keyed Toolkit composition (only resolved-Config Layers); tier-gate write-vs-read tools; structured api_key_required content — **shipped 2026-07-02** via goals/mcp-kit+uspto-mcp+mcp-host-retrofit except tier-gate wiring (PARTIAL, queued in goals/mcp-write-wall) and the untrusted-context description suffix (open) |
...
- **netNew:**
  - Conditional/credential-keyed Toolkit composition — build the Toolkit only from driver Layers whose Effect Config resolved successfully (CourtListener public vs USPTO/GovInfo/DOL keyed) — **[DONE, goals/uspto-mcp: `SourceAuth` + `ToolkitComposition` shipped and merged 2026-07-02]**
  - Tier-gating of write-vs-read tools at the candidate→approved wall (no real src implements requiresAuth/tier-gate today) — **[PARTIAL: `TierGate` primitive shipped in `@beep/mcp-kit` (fail-closed `dispatchWithTierGate` + `fromApprovedToolsPolicy`), but unwired against any live host; wiring it against a real write-capable host is goals/mcp-write-wall (queued)]**
  - Graceful 'API key missing' as structured content — a shared helper returning {error: api_key_required, tool, envVar, registrationUrl} instead of throwing — **[DONE, goals/uspto-mcp: `ApiKeyRequired` envelope shipped and merged 2026-07-02]**
  - Progressive-disclosure named field tiers (minimal/balanced/complete) projection / server-side response reshaping for context reduction (uspto_pfw field_configs pattern) — **[DONE, goals/uspto-mcp: `FieldTier` projector shipped and merged 2026-07-02]**
  - readOnly/idempotent tool-def hint conventions surfaced on tool metadata — **[PARTIAL: `annotateFourHints`/`ToolAnnotations` shipped and landed in `nlp-mcp`'s `StreamingToolkit` + `m365-mcp` (goals/mcp-host-retrofit, merged 2026-07-02); the "untrusted external context" description-suffix prompt-injection-hardening prose (USE WHEN/DO NOT USE, mike#7 nugget) is still open, tracked as an optional goals/mcp-write-wall deliverable]**
- **alreadyCovered:**
