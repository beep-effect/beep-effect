...
- **Coordinate with:** `packages/drivers/ecfr` (existing VERSION-only skeleton), `packages/drivers/govinfo`, `explorations/solo-firm-docketing`, `packages/drivers/runpod`, `packages/drivers/acp`. Recreate `@beep/courtlistener`, `@beep/dol`, and `@beep/federal-register` only when the delivery packet resumes — see `goals/honest-repo-signal/research/FOLLOW-UPS.md`.
- **Net-new (no existing home):**
  ...
  - Finish `@beep/ecfr` (VERSION-only today). Recreate courtlistener/dol/federal-register first; do not plan against phantom workspace packages.
...
- **Rationale:** `@beep/ecfr` is still a VERSION-only skeleton and govinfo is only partially scaffolded (Search HttpApi contract, no client/auth). CourtListener, DOL, and Federal Register packages were deleted 2026-08-13 — recreate them from `goals/honest-repo-signal/research/FOLLOW-UPS.md` when a product feature pulls. official-data-sync-foundation is a disjoint static-dataset (ISO/IANA/CLDR) goal, and solo-firm-docketing is a build-vs-buy decision packet frozen at its align review gate. The gold (an OpenAPI->SDK+MCP codegen pipeline plus auth/retry/cache patterns) is genuinely net-new infrastructure, coordinating with `@beep/ecfr`, govinfo, and the runpod/acp codegen precedent.
...
- **Rationale:** The `@beep/courtlistener` driver was deleted 2026-08-13 (was VERSION-only; recreate via `goals/honest-repo-signal/research/FOLLOW-UPS.md` if a product feature pulls the HTTP driver). No courts-db dataset, regex normalization dictionary, or span-gated court-string resolver exists anywhere in packages/, @beep/data, or goals. ip-law-knowledge-graph only defines abstract OWL Court/Jurisdiction graph nodes (PENDING), not a controlled vocabulary or resolver, and official-data-sync-foundation provides the dataset-sync mechanism but does not own court data. The vocabulary dataset plus resolver is a coherent net-new capability warranting its own packet that ingests via official-data-sync-foundation; do not treat the deleted courtlistener tree as a landing zone.
