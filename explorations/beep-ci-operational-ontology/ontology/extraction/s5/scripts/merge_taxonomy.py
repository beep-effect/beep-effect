...
def main() -> None:
    ...
    for t in terms:
        ...
        # boundary-faithful measurement parameters live on their property TERM
        # only; a class-side duplicate re-attaches the measurement to the
        # artifact (adversary finding + PR #907 review)
        if name in PARAMETER_RANGE_FIXES:
            ...
            kept = [prm for prm in t.get("parameters") or []
                    if prm.get("property") not in PARAMETER_RANGE_FIXES]
            if kept != (t.get("parameters") or []):
                t["parameters"] = kept
        if name == "publish":
    ...
    (S5 / "TAXONOMY.yaml").write_text(
        ...
        "# with the sitting-4 fidelity and ruling layers applied.\n"
        "# RATIFIED by the steward 2026-08-30 (DECISIONS.md, S5 sitting 4).\n"
