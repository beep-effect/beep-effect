...
def write_generated_yaml(
    ...
    *header_lines: str,
    ...
    """Write or check deterministic generated YAML with the S5 house banner."""
    ...
    if check:
        if not path.is_file() or path.read_bytes() != rendered.encode("utf-8"):
            raise SystemExit(f"{path.name} differs from generated output; run {script_name} without --check")
        print(f"CHECK: {path.name} matches generated output; no files changed")
    else:
