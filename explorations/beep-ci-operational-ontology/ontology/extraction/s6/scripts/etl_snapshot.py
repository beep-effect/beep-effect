...
# fresh captures must follow the running uid, never a hardcoded one. Hosts
# without os.getuid (Windows) can still run pinned reruns — only a fresh
# capture needs the live journal, and it fails with an actionable message.
_UID = os.getuid() if hasattr(os, "getuid") else None
SOURCE = Path(f"/tmp/beep-admit-uid-{_UID}/journal.ndjson") if _UID is not None else None
...
def main() -> None:
    ...
    if pinned:
        ...
        if SOURCE is None:
            raise SystemExit("fresh capture needs a per-uid admission root; this host has no os.getuid")
    ...
    snapshot_graph_text = "\n".join(ttl).rstrip() + "\n"
    graph_path.write_text(snapshot_graph_text)
    ...
    manifest_graph_path.write_text(manifest_graph_text)
    ...
    manifest = {
        ...
        "graph_sha256_12": sha256_12(snapshot_graph_text.encode()),
        "manifest_graph": f"graphs/{manifest_graph_name}",
        "manifest_graph_sha256_12": sha256_12(manifest_graph_text.encode()),
