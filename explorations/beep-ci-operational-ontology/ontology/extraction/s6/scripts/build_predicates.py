...
Check without writing: uv run --with pyyaml python build_predicates.py --check
"""
...
# Preserve the S6-era property surface while admitting later ratified terms
# from TAXONOMY (2026-09-10 amendment's named-baseline superset rule).
S6_BASELINE_PROPERTIES = {
    "activeTokens",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="build or check the S6 predicate registry")
    ...
    missing_properties = sorted(S6_BASELINE_PROPERTIES - set(taxonomy_properties))
    if missing_properties:
        raise SystemExit(f"taxonomy is missing S6 baseline properties: {missing_properties}")
