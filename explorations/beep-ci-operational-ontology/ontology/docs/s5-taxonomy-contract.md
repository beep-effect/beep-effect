...
2. Steward sittings (bulk-first): per-domain rulings for the 19 unratified
   ...
   machine rows.
3. Seats (one round): assembly lanes per kind cluster (codex `gpt-6-astra`, `medium`) build
   TAXONOMY.yaml over the accepted set; an independent adversary (codex `gpt-6-astra`, `medium`)
