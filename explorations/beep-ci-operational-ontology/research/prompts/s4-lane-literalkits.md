# S4 extraction lane: s4-literalkits
...
`LiteralKit(` call sites across `packages/**/src/**` (verification-relevant domains only), EXCLUDING these unit-5/7-owned files: Yeet/internal/Planner.ts, Yeet/internal/AttemptJournal.ts, Yeet/internal/Verdict.ts, internal/cli/TurboCache.ts, internal/repo-run/QualityScheduler.schemas.ts, internal/repo-run/QualityScheduler.ts, Yeet/internal/Handler.ts. Use `rg -n "LiteralKit\(" packages --type ts` to enumerate; judge verification-relevance against the frozen scope.

## Your output file (the ONLY file you may write)

`explorations/beep-ci-operational-ontology/ontology/extraction/s4/literalkits.yaml`
...
`cd explorations/beep-ci-operational-ontology/research/scripts && uv run --with pyyaml,rdflib python validate_packet.py --s4-lane ../../ontology/extraction/s4/literalkits.yaml`
