# S4 extraction lane: s4-yeet-internals
...
`packages/tooling/tool/cli/src/commands/Yeet/internal/Planner.ts` (`YeetProofTier`), `packages/tooling/tool/cli/src/commands/Yeet/internal/AttemptJournal.ts` (ring buffer, attempt events, attemptId), `packages/tooling/tool/cli/src/commands/Yeet/internal/Verdict.ts` (verdict/lane records, failedStepId, durationMs, elapsedMs), `packages/tooling/tool/cli/src/internal/cli/TurboCache.ts` (TurboCacheMode/TurboCachePlan postures). Candidates feed VerificationAttempt / CommittedFailure / actualWallMs / CachePosture carriers.

## Your output file (the ONLY file you may write)

`explorations/beep-ci-operational-ontology/ontology/extraction/s4/yeet-internals.yaml`
...
`cd explorations/beep-ci-operational-ontology/research/scripts && uv run --with pyyaml,rdflib python validate_packet.py --s4-lane ../../ontology/extraction/s4/yeet-internals.yaml`
