# S4 extraction lane: s4-package-topology
...
`explorations/beep-ci-operational-ontology/ontology/extraction/s4/inputs/topo-sort.txt` — the MATERIALIZED `bun run beep topo-sort` capture (sorted package NAMES + index; the first few non-package tokens are output artifacts — record them as an issue, do not invent meaning). Dependency EDGES come from workspace `packages/**/package.json` manifests (dependencies/devDependencies on `@beep/*`), which you read directly. Candidates feed PackageRef / dependsOn / dependsOnTransitive (CQ-004).

## Your output file (the ONLY file you may write)

`explorations/beep-ci-operational-ontology/ontology/extraction/s4/package-topology.yaml`
...
`cd explorations/beep-ci-operational-ontology/research/scripts && uv run --with pyyaml,rdflib python validate_packet.py --s4-lane ../../ontology/extraction/s4/package-topology.yaml`
