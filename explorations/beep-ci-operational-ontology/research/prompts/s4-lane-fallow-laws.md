# S4 extraction lane: s4-fallow-laws
...
`.fallowrc.jsonc`, `standards/fallow.boundaries.provenance.schema.json`, `standards/fallow.pilot.inventory.jsonc` (the fallow boundary configuration), `AGENTS.md` (repo root), `standards/ARCHITECTURE.md` — verification-relevant LAWS only (gates, boundaries, required proofs); prose evidence must be quoted verbatim.

## Your output file (the ONLY file you may write)

`explorations/beep-ci-operational-ontology/ontology/extraction/s4/fallow-laws.yaml`
...
`cd explorations/beep-ci-operational-ontology/research/scripts && uv run --with pyyaml,rdflib python validate_packet.py --s4-lane ../../ontology/extraction/s4/fallow-laws.yaml`
