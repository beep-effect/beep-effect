...
**Timeline note for the steward** *(corrected per Ruling 8 — the as-grilled draft also listed the emission-v2 PR here)*: the only repo change gating the Stage B CAPTURE is the v3 journal events PR (R1-1/R2-2), plus the operational clocks (yeet traffic to populate v3 events and proof-ledger rows). The S7 emission-v2 PR (R2-6) is a parallel workstream gating the ordering-cluster EVIDENCE consumed at the run itself, never the capture. Checkout-identity has NO instrumentation gate and can pin first under the staged plan.
