...
| D8 | `.entity.ts` cluster kit, `.schema(s).ts` roles, one-file-per-operation `contracts/<Op>.contract.ts` (`frs-09`, `frs-10`, `frs-14`) | 14/22 were empty shells; v4's `.commands/.queries` + driver-neutral `.http/.rpc/.tools` declarations cover the same ground without a file per verb | use-cases declarations, server handlers |
