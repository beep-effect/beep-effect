...
| PROV-O substrate (`ProvDateTime`, `Revision`, `wasRevisionOf`/`wasDerivedFrom`) | `@beep/rdf` (`packages/foundation/modeling/rdf/src/Prov.ts`) | **reuse/extend** — project supersession chain to PROV (note: no public `ProvDateTimeChecks` export) |
