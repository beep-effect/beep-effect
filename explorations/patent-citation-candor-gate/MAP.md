...
| Entity identity | **NET-NEW (required registration):** `PatentCitationEventId` / `CandorDispositionId` in `packages/shared/domain/src/identity/LawPractice` — every law-practice entity takes its id from there (precedent: `PriorArtReferenceId`, `LawPractice.ts:293`, consumed at `PriorArtReference.model.ts:51`). |
