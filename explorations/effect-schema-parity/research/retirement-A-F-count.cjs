...
const out = __dirname;
const base = "packages/foundation/modeling/schema/src/";
    ...
    if (ts.isExportDeclaration(n)) {
      const target =
        n.moduleSpecifier && ts.isStringLiteral(n.moduleSpecifier)
          ? path.normalize(path.join(path.dirname(f), n.moduleSpecifier.text))
          : null;
      if (n.exportClause && ts.isNamespaceExport(n.exportClause)) {
        result.set(n.exportClause.name.text, conceptOf(target || f));
        continue;
      }
      ...
      if (n.exportClause && ts.isNamedExports(n.exportClause))
        for (const e of n.exportClause.elements)
          result.set(e.name.text, imported.get((e.propertyName || e.name).text) || conceptOf(target || f));
      ...
      if (ts.isVariableStatement(n))
        for (const d of n.declarationList.declarations)
          if (ts.isIdentifier(d.name)) result.set(d.name.text, conceptOf(f));
          else {
          }
...
const root = exported(`${base}index.ts`);
...
for (const f of candidates) {
  ...
  for (const n of sf.statements) {
    ...
    if (!spec?.startsWith("@beep/schema")) continue;
    if (spec.startsWith("@beep/schema/")) {
      let c = spec.slice("@beep/schema/".length).split("/")[0];
      if (c === "FileDiff") c = "FileDiff.schema";
      add(c, f);
      continue;
    }
    if (spec !== "@beep/schema") continue;
    const cl = ts.isImportDeclaration(n) ? n.importClause?.namedBindings : n.exportClause;
    if (cl && (ts.isNamespaceImport(cl) || ts.isNamespaceExport(cl))) {
      namespaces.add(cl.name.text);
      if (ts.isNamespaceExport(cl)) unresolved.push({ file: f, kind: "root namespace reexport" });
    } else if (cl && (ts.isNamedImports(cl) || ts.isNamedExports(cl)))
      for (const e of cl.elements) add(root.get((e.propertyName || e.name).text), f);
  ...
  const walk = (n) => {
    ...
    if (ts.isElementAccessExpression(n) && ts.isIdentifier(n.expression) && namespaces.has(n.expression.text)) {
      if (n.argumentExpression && ts.isStringLiteral(n.argumentExpression)) add(root.get(n.argumentExpression.text), f);
      else unresolved.push({ file: f, kind: "computed namespace access" });
    }
    if (ts.isImportTypeNode(n) && ts.isLiteralTypeNode(n.argument) && ts.isStringLiteral(n.argument.literal)) {
      const spec = n.argument.literal.text;
      if (spec.startsWith("@beep/schema/")) add(spec.slice(13).split("/")[0], f);
      else if (spec === "@beep/schema" && n.qualifier) {
        let q = n.qualifier;
        while (ts.isQualifiedName(q)) q = q.left;
        add(root.get(q.text), f);
      }
    }
...
const evidence = Object.fromEntries(
  concepts.map((c) => [
    c,
    {
      count: hits[c].size,
      files: [...hits[c]].sort(),
      rootNames: [...root].filter(([n, x]) => x === c).map(([n]) => n),
    },
  ])
);
fs.writeFileSync(
  path.join(out, "retirement-A-F-consumers.json"),
  `${JSON.stringify(
    {
      method:
        "rg candidate census plus TypeScript syntax import/root member resolution; no transitive reexport closure",
      concepts: evidence,
      unresolved,
    },
    null,
...
for (const [c, e] of Object.entries(evidence)) console.log(`${c}\t${e.count}\t${e.rootNames.join(",")}`);
