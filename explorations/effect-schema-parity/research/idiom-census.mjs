...
const files = spawnSync("rg", ["--files", "--hidden", "packages", "apps"], { encoding: "utf8", maxBuffer: 20000000 })
  .stdout.trim()
  .split("\n")
  .filter(
    (p) =>
      /^(packages|apps)\/.+\/src\/.+\.(ts|tsx)$/.test(p) &&
      !/(^|\/)(node_modules|dist|\.repos|fixtures|__fixtures__|test|tests|__tests__)(\/|$)/.test(p) &&
      !/\.(test|spec|tst)\.[cm]?tsx?$/.test(p)
  )
  .sort();
const patterns = {
  ...
  F10: "transformOrFail",
  F11: "documentation:",
  F12: "~standard|toStandardSchemaV1|standardSchemaResolver|effectTsResolver",
...
for (const [id, regex] of Object.entries(patterns)) {
  ...
  const matches = run.stdout
    ...
    .map((x) => ({ path: x.data.path.text, line: x.data.line_number, text: x.data.lines.text.trimEnd() }))
    .sort((a, b) => a.path.localeCompare(b.path) || a.line - b.line);
  rows.push({ id, regex, lines: matches.length, files: new Set(matches.map((x) => x.path)).size, matches });
}
if (!selectedId)
  writeFileSync(
    `${root}idiom-census.json`,
    `${JSON.stringify(
      {
        scopeFiles: files.length,
        scope: files,
        unit: "matching source lines; comments and generated source retained; not confirmed violations",
        rows,
      },
      null,
      2
...
for (const r of rows)
  console.log(
    `${r.id}\t${r.lines}\t${r.files}\t${r.matches
      .slice(0, 3)
      .map((x) => `${x.path}:${x.line}: ${x.text.trim()}`)
      .join("\n")}`
  );
