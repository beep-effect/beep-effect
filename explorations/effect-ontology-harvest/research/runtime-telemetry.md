...
- **beep overlap & design delta:** NOT FOUND after live source/barrel searches for an embedding provider or embedding capability. The closest confirmed pattern is professional-desktop's config-selected fixture/Anthropic runtime Layers (`apps/professional-desktop/src/runtime/Layer.ts:114-160`); effect-ontology adds a useful multi-provider dependency-normalization example, but its assertions should not be copied.
- **Goal linkage:** none
...
- **beep overlap & design delta:** Partial overlap is confirmed: professional-desktop already owns PGlite release through Layer scope and delegates lifetime to `BunRuntime.runMain(Layer.launch(...))` (`apps/professional-desktop/src/runtime/Pglite.ts:13-22`; `apps/professional-desktop/server/main.ts:105-107`). A reusable drain coordinator was NOT FOUND, but Graphiti tooling has the stronger scoped signal-listener and queue-drain pattern using `Effect.acquireRelease` and a deferred shutdown gate (`packages/tooling/tool/cli/src/commands/Graphiti/internal/ProxyRuntime.ts:107-164`). Harvest the admission/accounting seam, not the manual process control.
...
- **v3→v4 notes:** Replace `Effect.Service` and plain config/error shapes with `Context.Service` plus `S.Class`, use `Duration` throughout, surface timeout as an explicit outcome, and register signals through scoped acquisition or the runtime teardown hook instead of calling `process.exit` (`packages/@core-v2/src/Runtime/Shutdown.ts:11-38,70-148`; `packages/tooling/tool/cli/src/commands/Graphiti/internal/ProxyRuntime.ts:107-164`).
- **Effort:** M
- **Verdict hint:** adapt-improve
