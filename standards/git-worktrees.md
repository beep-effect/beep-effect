...
`.idea/` needs no restore step: the curated project config (module file, exclusions,
inspection profile) is tracked in Git and arrives with the checkout, while machine-local
state (`workspace.xml`, `effect.intellij.xml`) stays ignored via the whitelist
`.idea/.gitignore` and is regenerated by the IDE per worktree.
...
- `worktree new <name>` creates `<worktrees-root>/<name>` on a new branch
  ...
  `.claude/settings.local.json`, and `CLAUDE.local.md` (`.idea/` is Git-tracked
  and needs no copy). It prints a
...
- `.turbo`, `.sst`, `.beep`, and `node_modules` are intentionally local to
  ...
  ignored machine-local files stay per-worktree.
