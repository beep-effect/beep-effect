...
import { Match, MutableHashMap, Option } from "effect";
...
const navigateHeaderPrefix = (root: SemNode, keyPath: ReadonlyArray<TomlKey>): SemNode => {
  ...
  for (let i = 0; i < keyPath.length - 1; i++) {
    const key = keyPath[i];
    const existing = Option.getOrUndefined(MutableHashMap.get(current.table, key.value));
    ...
    current = Match.value(existing.kind).pipe(
      Match.when("table-explicit", () => existing),
      Match.when("table-implicit", () => existing),
      Match.when("table-dotted", () => existing),
      Match.when("array-tables", () => existing.elements[existing.elements.length - 1]),
      Match.when("inline", () => raise("InlineTableExtended", `inline table "${key.value}" cannot be extended`, key)),
      Match.when("static-array", () =>
        raise("ArrayOfTablesConflict", `"${key.value}" is a static array, not an array of tables`, key)
      ),
...
const openTable = (root: SemNode, keyPath: ReadonlyArray<TomlKey>): SemNode => {
  ...
  const existing = Option.getOrUndefined(MutableHashMap.get(parent.table, key.value));
  if (existing === undefined) {
...
const nodeForValue = (value: TomlValueNode, context: Context): SemNode => {
  if (TomlInlineTable.is(value)) {
    return inlineNode(value, context);
  }
  if (TomlArray.is(value)) {
...
const checkArrayItem = (item: TomlValueNode, context: Context): void => {
  if (TomlInlineTable.is(item)) {
    inlineNode(item, context);
    return;
  }
  if (TomlArray.is(item)) {
...
export const analyze: {
  ...
  (expressions: ReadonlyArray<TomlExpression>, visitor?: SemanticVisitor): void => {
    ...
    for (const expression of expressions) {
      if (TomlTrivia.is(expression)) {
        continue;
      }
      if (TomlTableHeader.is(expression)) {
      ...
      if (TomlArrayTableHeader.is(expression)) {
