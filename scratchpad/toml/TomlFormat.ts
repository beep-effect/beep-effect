...
import { Effect, Match, MutableHashMap, Option, Result, Schema } from "effect";
import * as P from "effect/Predicate";
import type { TomlErrorCodeRaw } from "./internal/diagnostics.ts";
import { isRawTomlError, RawTomlError, TomlErrorCode } from "./internal/diagnostics.ts";
import { GuardExceeded, isGuardExceeded, MAX_NESTING_DEPTH } from "./internal/limits.ts";
...
import type { TomlPath, TomlSegment } from "./TomlEdit.ts";
import { TomlEdit, TomlRange } from "./TomlEdit.ts";
 ...
 *
 * **Example** (Guard a structural range literal)
 *
 * ```ts
 * import { TomlRangeLike } from "@beep/scratchpad/toml"
 * import { Schema } from "effect"
 *
 * console.log(Schema.is(TomlRangeLike)({ offset: 0, length: 12 })) // true
 * ```
 *
 ...
 * @since 0.0.0
 */
export const TomlRangeLike = Schema.Union([TomlRange, Schema.Struct(TomlRange.fields)]).pipe(
  $I.annoteSchema("TomlRangeLike", {
    description: "A TOML formatting range accepted as either the canonical class or a structural literal.",
  })
);

/**
 * Decoded TOML formatting range accepted by formatting APIs.
 *
 * @see {@link TomlRangeLike} for the runtime schema.
 * @category type-level
 * @since 0.0.0
 */
export type TomlRangeLike = typeof TomlRangeLike.Type;
 ...
 * import { TomlFormat, TomlModificationError } from "@beep/scratchpad/toml"
 *
 ...
 * if (Result.isFailure(outcome) && outcome.failure instanceof TomlModificationError) {
 *   console.log(outcome.failure.diagnostic.code) // "DottedKeyConflict"
 * }
 * ```
 *
...
const formatTail = (source: string, emit: FormatEmitter, expr: TomlExpression, contentEnd: number): void => {
  let nlStart = expr.offset + expr.length;
...
class TomlFormatInvariantError extends Schema.TaggedError<TomlFormatInvariantError>($I`TomlFormatInvariantError`)(
...
const intoTable = (node: ResNode | undefined): ResTable => {
  ...
  throw TomlFormatInvariantError.make({ detail: "semantic pass admitted a value where a table was navigated" });
};
...
const buildSemanticIndex = (
  ...
  const navigateHeaderPrefix = (path: ReadonlyArray<string>): ResTable => {
    ...
    for (let i = 0; i < path.length - 1; i++) {
      let child = Option.getOrUndefined(MutableHashMap.get(current.entries, path[i]));
      if (child === undefined) {
        child = mkTable("implicit");
        MutableHashMap.set(current.entries, path[i], child);
      }
  ...
  analyze(expressions, {
    onTableStart: (path, header) => {
      ...
      const existing = Option.getOrUndefined(MutableHashMap.get(parent.entries, name));
    ...
    onArrayTableStart: (path, _index, _header) => {
      ...
      if (existing !== undefined && existing.kind === "array-tables") {
        ...
        MutableHashMap.set(parent.entries, name, {
          kind: "array-tables",
          elements: [mkTable("element", sectionCounter)],
        });
      }
    },
    onKeyValue: (path, expr) => {
      ...
      for (let i = 0; i < path.length - names.length; i++) {
        current = intoTable(Option.getOrUndefined(MutableHashMap.get(current.entries, path[i])));
      }
      for (let j = 0; j < names.length - 1; j++) {
        let child = Option.getOrUndefined(MutableHashMap.get(current.entries, names[j]));
        if (child === undefined) {
          child = mkTable("dotted", sectionCounter, names.slice(0, j + 1));
          MutableHashMap.set(current.entries, names[j], child);
        }
        current = intoTable(child);
      }
      MutableHashMap.set(current.entries, names[names.length - 1], { kind: "value", node: expr.value, expr });
    },
  });
...
class ModifyFailure extends Schema.TaggedError<ModifyFailure>($I`ModifyFailure`)(
  ...
  $I.annote("ModifyFailure", {
    description: "Internal TOML modification failure before public diagnostic materialization.",
  })
) {}
...
const step = (cur: Cursor, segment: TomlSegment): Cursor => {
  if (cur.t === "table") {
    ...
    const child = Option.getOrUndefined(MutableHashMap.get(cur.table.entries, key));
    if (child === undefined) {
      return failResolve(
        "DottedKeyConflict",
...
const terminal = (cur: Cursor, segment: TomlSegment, value: unknown, ctx: ModifyContext): Array<RawEdit> => {
  if (cur.t === "table") {
    const key = String(segment);
    const existing = Option.getOrUndefined(MutableHashMap.get(cur.table.entries, key));
    if (value === undefined) {
...
type ModificationResolutionError = ModifyFailure | RawTomlError | GuardExceeded;

const resolveModification = (
  root: ResTable,
  path: TomlPath,
  value: unknown,
  ctx: ModifyContext
): Result.Result<ReadonlyArray<RawEdit>, ModificationResolutionError> => {
  try {
    let cursor: Cursor = { t: "table", table: root };
    for (let i = 0; i < path.length - 1; i++) {
      cursor = step(cursor, path[i]);
    }
    return Result.succeed(terminal(cursor, path[path.length - 1], value, ctx));
  } catch (defect) {
    if (isModifyFailure(defect) || isRawTomlError(defect) || isGuardExceeded(defect)) {
...
export class TomlFormat {
  ...
  static readonly modify = Effect.fn("TomlFormat.modify")(function* (
    ...
    const raw = yield* Effect.fromResult(resolveModification(root, path, value, ctx)).pipe(
      Effect.mapError(materializeModificationError(text))
    );
    return raw.map((edit) =>
