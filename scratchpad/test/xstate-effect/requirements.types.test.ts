import { type createEffectActor, fromEffect, type RequirementsFrom, setupEffect } from "@beep/scratchpad/xstate-effect";
import { describe, it } from "@effect/vitest";
import { Effect } from "effect";
import * as Context from "effect/Context";
import { type AnyActorLogic, createMachine, setup } from "xstate";
...
type ActorRequirements<TLogic extends AnyActorLogic> =
  ReturnType<typeof createEffectActor<TLogic>> extends Effect.Effect<unknown, unknown, infer R> ? R : never;

class Alpha extends Context.Service<Alpha, { a: number }>()(
  "@beep/scratchpad/test/xstate-effect/requirements.types.test/Alpha"
) {}
class Beta extends Context.Service<Beta, { b: string }>()(
  "@beep/scratchpad/test/xstate-effect/requirements.types.test/Beta"
) {}

const alphaEffect = Alpha.use((alpha) => Effect.succeed(alpha.a));
const betaEffect = Beta.use((beta) => Effect.succeed(beta.b));

type RequirementsOfEffect<T> = T extends Effect.Effect<unknown, unknown, infer R> ? R : never;
...
describe("RequirementsFrom", () => {
  it("collects requirements from Effect logic directly", () => {
    true satisfies Equals<RequirementsFrom<typeof alphaLogic>, AlphaRequirement>;
    false satisfies IsNever<RequirementsFrom<typeof alphaLogic>>;
    true satisfies Includes<ActorRequirements<typeof alphaLogic>, AlphaRequirement>;
  });
  ...
  it("collects requirements from a registered Effect actor", () => {
    ...
    true satisfies Includes<ActorRequirements<typeof machine>, AlphaRequirement>;
  });
  ...
  it("collects requirements through a registered child machine", () => {
    ...
    const probe: IsNever<ActorRequirements<typeof parent>> extends true ? "NESTED_R_IS_NEVER" : "nested ok" =
      "NESTED_R_IS_NEVER";
    void probe;
  });
  ...
  it("unions requirements from two different actors", () => {
    ...
    true satisfies Equals<RequirementsFrom<typeof parent>, AlphaRequirement | BetaRequirement>;
...
describe("RequirementsFrom (inline invoke.src)", () => {
  ...
  it("collects requirements from an array of invoke configs", () => {
    ...
    true satisfies Equals<RequirementsFrom<typeof machine>, AlphaRequirement | BetaRequirement>;
  });
  ...
  it("no longer infers `never` for inline Effect logic", () => {
    ...
    const probe: IsNever<RequirementsFrom<typeof inline>> extends true ? "INLINE_R_IS_NEVER" : "inline ok" =
      "INLINE_R_IS_NEVER";
    void probe;
  });
