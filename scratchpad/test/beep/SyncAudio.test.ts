...
describe("SyncAudio", () => {
  it("keeps len as len and defaults duration and spans at construction", () => {
    const span = Effect.runSync(S.decodeUnknownEffect(ConversationAudioSpanInfo)({
      fileId: "file-1",
      wallOffset: 1.5,
      artifactOffset: 0,
      len: 3,
    ...
    const file = Effect.runSync(S.decodeUnknownEffect(AudioFileUrlInfo)({ id: "file-1", status: "ready", duration: 1 }));
  ...
  it("decodes present, null, and missing optional audio fields", () => {
    const precache = Effect.runSync(S.decodeUnknownEffect(AudioPrecacheResponse)({
      status: "queued",
      message: null,
      audioFileCount: 2,
    }));
    ...
    const urls = Effect.runSync(S.decodeUnknownEffect(AudioUrlsResponse)({
    ...
    const polling = Effect.runSync(S.decodeUnknownEffect(AudioUrlsResponse)({
      audioFiles: [],
      conversationAudio: {
        status: "ready",
        spans: [],
