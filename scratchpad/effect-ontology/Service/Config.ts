...
const LlmConfig = Config.nested("LLM")(Config.all({
  provider: Config.literals(["anthropic", "openai", "google"], "PROVIDER").pipe(
...
const StorageConfig = Config.nested("STORAGE")(Config.all({
  type: Config.literals(["local", "gcs", "memory"], "TYPE").pipe(
...
export const ConfigService = Context.Service<ConfigService>($I`ConfigService`)
