...
export class ClaimService extends Context.Service<ClaimService>()($I`ClaimService`, {
  make: Effect.gen(function*() {
     ...
     */
    const createClaim =
      Effect.fn(function*(input: CreateClaimInput)  {
     ...
     */
    const deprecateClaim =
      Effect.fn(function* (claimId: string, reason: string, correctionId?: string)  {
     ...
     */
    const addClaimToStore =
      Effect.fn(function* (store: RdfStore, claim: ClaimRow, graphUri?: string) {
     ...
     */
    const claimsToTurtle =
      Effect.fn(function*(claims: Array<ClaimRow>, graphUri?: string)  {
    ...
    static readonly Default = Layer.effect(this, this.make).pipe(Layer.provide([ClaimRepository.Default, RdfBuilder.Default]));
}
