...
import { Chunk, Duration, Effect, type Scope, Context, Layer } from "effect"
...
import { type BlankNode as BlankNodeType, type IRI, Literal, Quad, type RdfTerm } from "../Domain/Rdf/Types.ts"
...
const n3TermToDomainTerm = (term: N3.Term): RdfTerm => {
  if (term.termType === "NamedNode") {
    ...
    return Literal.make({
      ...
      datatype: O.fromNullishOr(term.datatype ? (term.datatype.value as IRI) : undefined)
    })
  } else {
 ...
 * @category Services
 */
export class RdfBuilder extends Context.Service<RdfBuilder>()(
  $I`RdfBuilder`,
  {
      ...
      return {
        ...
        toTurtle: (store: RdfStore) =>
          Effect.callback<string, SerializationFailed>((resume) => {
            ...
            Effect.timeoutOrElse({ duration: Duration.seconds(30), orElse: () => Effect.fail(SerializationFailed.make({
