...
import { Cause, Context, DateTime, Effect, Exit, Hash, Layer, Match, Option, Order, Ref, Schedule, Schema } from "effect"
...
import { BatchState, type DocumentStatus } from "../Domain/Model/BatchWorkflow.ts"
...
export const BatchExtractionWorkflow = Workflow.make("batch-extraction", {
  ...
  suspendedRetrySchedule: Schedule.max([Schedule.exponential("1 second"), Schedule.recurs(5)]).pipe(
    Schedule.jittered
  )
})
...
const parseManifest = (raw: string) => Schema.decodeUnknownSync(Schema.fromJsonString(BatchManifest))(raw)
...
export const handleWorkflowResult = Effect.fn("WorkflowOrchestrator.handleWorkflowResult")(
  function*<A, E>(
    executionId: string,
...
export const pollToBatchState = Effect.fn("WorkflowOrchestrator.pollToBatchState")(
  ...
  (payload) =>
    Effect.gen(function*() {
      ...
      const manifestRaw = yield* storage.get(manifestKey).pipe(
        Effect.flatMap((opt) => expectValue(O.fromNullishOr(opt), manifestKey))
      )
      ...
      const emitState = (state: BatchState) =>
        Effect.gen(function*() {
          ...
          Effect.catch((error) => Effect.logWarning("Failed to publish batch state", { batchId, error }))
        )
      ...
      const runWorkflow = Effect.gen(function*() {
        ...
        const enrichedManifest = enrichedManifestRaw
          ? Schema.decodeUnknownSync(Schema.fromJsonString(EnrichedManifest))(enrichedManifestRaw)
          : null
          ...
          (doc) =>
            Effect.gen(function*() {
              ...
              yield* Ref.update(documentStatusesRef, (statuses) =>
                A.map(statuses, (s) =>
                  s.documentId === doc.documentId
                    ? { documentId: s.documentId, status: "processing" as const, startedAt }
                    : s
                ))
              ...
              const extractingState: BatchState = {
                ...
                currentDocumentId: O.some(doc.documentId as DocumentId),
        ...
        if (!validationResult.conforms) {
          yield* eventBus.publishExtractionEvent("ValidationFailed", {
            ...
            reportUri: Option.fromNullishOr(validationResult.reportUri),
        ...
        const claimPersistenceResult = yield* makeClaimPersistenceActivity({
          ...
          documentGraphUris: A.map(successfulResults, (r) => r.graphUri),
          targetNamespace: manifest.targetNamespace,
          documentMetadata: O.some(A.map(manifest.documents, (doc) => ({
            documentId: doc.documentId,
            sourceUri: doc.sourceUri,
            eventTime: O.none(),
            headline: O.none()
        ...
        yield* eventBus.publishExtractionEvent("ExtractionCompleted", {
          ...
          outputUri: Option.fromNullishOr(complete.canonicalGraphUri),
...
export interface WorkflowOrchestrator {
   ...
   */
  readonly poll: (executionId: string) => Effect.Effect<Option.Option<Workflow.Result<BatchState, string>>>
...
export const WorkflowOrchestrator = Context.Service<WorkflowOrchestrator>($I`WorkflowOrchestrator`)
