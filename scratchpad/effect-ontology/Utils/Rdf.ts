...
import { IRI, type IRI as IriValue } from "../Domain/Model/shared.ts"
...
const typeQuad = (
  ...
  prefixes: RdfPrefixes,
  builders: N3TermBuilders
): N3.Quad => builders.quad(subject, builders.namedNode(`${prefixes.rdf}type`), builders.namedNode(typeIri))
...
export const entityToQuads = (
  ...
  const attributeQuads = A.map(Rec.toEntries(entity.attributes), ([predicate, value]) => {
    const predicateIri = IRI.is(predicate)
      ? predicate
      : `${prefixes.schema ?? baseNamespace}${predicate}`
