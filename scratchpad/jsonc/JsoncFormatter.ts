...
export class JsoncFormatter {
   ...
   *
   * **Example** (Pretty-print via computed edits)
   *
   * ```ts
   * import { JsoncEdit, JsoncFormatter } from "@beep/scratchpad/jsonc";
   *
   * const text = '{"a":1,"b":2}';
   * const edits = JsoncFormatter.format(text);
   * const formatted = JsoncEdit.applyAll(text, edits);
   *
   ...
   *
   * **Example** (Pretty-print compact JSONC in one step)
   *
   * ```ts
   * import { JsoncFormatter } from "@beep/scratchpad/jsonc";
   *
   * console.log(JsoncFormatter.formatToString('{"a":1,"b":2}')); // '{\n  "a": 1,\n  "b": 2\n}'
   * ```
   *
   * @param text - The JSONC source to format.
