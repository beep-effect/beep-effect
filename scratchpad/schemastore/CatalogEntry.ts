...
const CONFIG_EXTENSIONS = HashSet.make(
  "cfg",
  "conf",
  "env",
  "ini",
...
const GENERIC_BASENAMES = HashSet.make("conf", "config", "configuration", "options", "settings");
...
const lintPattern = (pattern: string): ReadonlyArray<CatalogLintFinding> => {
  ...
  if (stem === "*" && HashSet.has(CONFIG_EXTENSIONS, extension)) {
    findings.push(
  ...
  } else if (HashSet.has(GENERIC_BASENAMES, stem.toLowerCase()) && HashSet.has(CONFIG_EXTENSIONS, extension)) {
    findings.push(
      CatalogLintFinding.make({
        check: "GenericFileMatch",
        pattern,
...
export class CatalogEntry extends Schema.Class<CatalogEntry>($I`CatalogEntry`)(
   ...
   *
   * **Example** (Assemble an unversioned catalog row)
   *
   * ```ts
   * import { CatalogEntry } from "@beep/scratchpad/schemastore"
   *
  ...
  static assemble(options: {
    ...
    const urls: CatalogUrls = SchemaVersioning.catalogUrls({
      ...
      ...O.getSomesStruct({ versions: O.fromUndefinedOr(options.versions) }),
    });
    return CatalogEntry.make({
      ...
      url: urls.url,
      ...O.getSomesStruct({ versions: O.fromUndefinedOr(urls.versions) }),
    });
  }
   ...
   *
   * **Example** (Flag a generic fileMatch)
   *
   * ```ts
   * import { CatalogEntry } from "@beep/scratchpad/schemastore"
   *
   ...
   *   baseUrl: "https://example.com/schemas",
   * })
   * console.log(entry.lint().map((finding) => finding.check))
   * // => ["GenericFileMatch"]
   * ```
   *
   * @since 0.0.0
   ...
   *
   * **Example** (Flag braces before an entry exists)
   *
   * ```ts
   * import { CatalogEntry } from "@beep/scratchpad/schemastore"
   *
   * console.log(CatalogEntry.lintFileMatch(["config.{json,yml}"]).map((finding) => finding.check))
   * // => ["ComplexFileMatch"]
   * ```
   *
   * @since 0.0.0
