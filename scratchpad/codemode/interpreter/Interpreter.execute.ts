...
import { DataValue, type DataValue as DataValueType } from "../Codemode.data.ts";
import { DiagnosticModel, FailureModel, ResultModel, SuccessModel } from "../Codemode.result.ts";
import type { ExecuteOptions, ExecutionLimits } from "../Codemode.service.ts";
...
const boundOutput = (result: ResultModel, maxOutputBytes: number): ResultModel =>
  ResultModel.match(result, {
    Success: (success) => {
      ...
      const logs = boundLogs(O.getOrElse(success.logs, A.empty), Math.max(0, maxOutputBytes - valueBytes));
    ...
    Failure: (failure) => {
      ...
      const logs = boundLogs(O.getOrElse(failure.logs, A.empty), Math.max(0, maxOutputBytes - error.bytes));
