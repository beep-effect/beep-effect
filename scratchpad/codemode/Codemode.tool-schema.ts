...
const effectNumberSentinel = (schema: SubSchema): boolean =>
  ...
  pipe(
    ...
    O.exists(
      (values) =>
        A.length(values) === 1 &&
        A.some(values, (value) => value === "NaN" || value === "Infinity" || value === "-Infinity")
    )
  );
  ...
  return A.match(concrete, {
    ...
    onNonEmpty: ([head, ...tail]) =>
      A.isReadonlyArrayEmpty(tail)
        ...
        : pipe(
            concrete,
            A.map((member) => (pipe(member, Str.includes(" | ")) ? `(${member})` : member)),
            A.join(" & ")
          ),
  });
};
...
const definitionName = (ref: string): O.Option<string> =>
  pipe(
    ...
    O.filter((segment) => Str.isNonEmpty(segment) && !pipe(segment, Str.includes("/"))),
...
const nestedContext = (node: Node, context: RenderContext): RenderContext =>
  pipe(
    ...
    O.match({
      ...
      onSome: (definitions) => ({
        ...context,
        definitions: R.union(context.definitions, definitions, (_left, right) => right),
      }),
    })
  );
        ...
        O.flatMap((name) =>
          HashSet.has(seen, name)
            ...
            : pipe(
                ...
                O.filter((target) => !hasUnresolvedRef(target, definitions, HashSet.add(seen, name), nextVisited))
              )
        ),
        O.isNone
      )
...
const docTags = (schema: Node): ReadonlyArray<string> =>
  pipe(
    A.make(
      ...
      pipe(
        schema.format,
        O.map((format) => `@format ${format}`)
      ),
      pipe(
        schema.minItems,
        O.map((value) => `@minItems ${value}`)
      ),
      pipe(
        schema.maxItems,
        O.map((value) => `@maxItems ${value}`)
      )
    ),
    A.getSomes
  );
...
const jsdoc = (description: O.Option<string>, tags: ReadonlyArray<string>, pad: string): string => {
  const lines = pipe(
    description,
    O.map(Str.split("\n")),
    O.getOrElse(A.empty),
...
const renderSchema = (
  ...
  if (O.isSome(schema.$ref)) {
    ...
    return intersection(
      A.make(
        renderSchema(target.value, nested, depth, HashSet.add(seen, name.value)),
        renderSchema({ ...schema, $ref: O.none() }, nested, depth + 1, seen)
      )
    );
  }
  ...
  if (O.isSome(alternatives)) {
    ...
    if (
      ...
      A.every(members, (item) => (!P.isBoolean(item) && hasType(item, "number")) || effectNumberSentinel(item))
    ) {
      return "number";
    }
    if (
    ...
    const rendered = A.map(members, (item) => renderSchema(item, nested, depth + 1, seen));
    if (A.some(rendered, (member) => member === "unknown")) return "unknown";
    return intersection(
      A.make(
        pipe(rendered, A.join(" | ")),
        renderSchema({ ...schema, anyOf: O.none(), oneOf: O.none() }, nested, depth + 1, seen)
      )
    );
  }

  if (O.isSome(schema.allOf)) {
    const members = A.map(schema.allOf.value, (item) => renderSchema(item, nested, depth + 1, seen));
    if (A.some(schema.allOf.value, (item) => hasUnresolvedRef(item, nested.definitions))) {
      return "unknown";
    }
    return intersection([renderSchema({ ...schema, allOf: O.none() }, nested, depth + 1, seen), ...members]);
  }
  ...
  if (O.isSome(schema.type) && !P.isString(schema.type.value)) {
    return pipe(
      schema.type.value,
      A.map((type) => renderSchema({ ...schema, type: O.some(type) }, nested, depth + 1, seen)),
      A.join(" | ")
    );
  }
  ...
  if (type === "object" || O.isSome(schema.properties)) {
    ...
    const indexType = pipe(
      ...
      O.map((additional) => renderSchema(additional, nested, depth + 1, seen))
    );
    ...
    if (!context.pretty) {
      ...
      return A.isReadonlyArrayEmpty(members) ? "{}" : `{ ${pipe(members, A.join("; "))} }`;
    }
    ...
    const rendered = pipe(
      ...
      O.match({
        onNone: () => lines,
        onSome: (value) => A.append(lines, `${pad}[key: string]: ${value},`),
      })
    );
    return `{\n${pipe(rendered, A.join("\n"))}\n${pipe("  ", Str.repeat(depth))}}`;
  }
...
export const toTypeScript: {
  ...
  (schema: S.Top, decoded = false, pretty = false): string =>
    pipe(
      Result.try({
...
export class InputProperty extends S.Class<InputProperty>($I`InputProperty`)(
  ...
  static readonly new = (name: string, description: O.Option<string>, required: boolean): InputProperty =>
    InputProperty.make({ name, description, required });
}
...
const toolInputDocument = (tool: Tool.Any): Result.Result<ToolInputDocument, unknown> =>
  pipe(
    Result.try({
  ...
  pipe(
    ...
    Result.map(({ definitions, schema: root }) => {
      ...
      return pipe(
        ...
        O.getOrElse(A.empty),
        A.map(([name, value]) =>
          InputProperty.new(name, P.isBoolean(value) ? O.none() : value.description, HashSet.has(required, name))
        )
      );
    }),
...
export const inputTypeScript: {
  ...
  (args) => P.isObject(args[0]),
  (tool: Tool.Any, pretty = false): string => jsonSchemaToTypeScript(Tool.getJsonSchema(tool), pretty)
);
...
export const outputTypeScript: {
  ...
  (args) => P.isObject(args[0]),
  (tool: Tool.Any, pretty = false): string => {
