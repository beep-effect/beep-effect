...
const $I = $ScratchpadId.create("MemoryFileSystem/internal/memoryFileSystem");
...
// models
// =============================================================================
const Inode = S.Finite.pipe(
  S.brand("MemoryFileSystemInode"),
  $I.annoteSchema("Inode", {
...
declare namespace Inode {
  type Encoded = typeof Inode.Encoded;
}

const RootInode = Inode.make(1);

const FileDescriptor = S.Finite.pipe(
  S.brand("MemoryFileSystemFileDescriptor"),
  $I.annoteSchema("FileDescriptor", {
...
declare namespace FileDescriptor {
  type Encoded = typeof FileDescriptor.Encoded;
}
...
class InodeMetadata extends S.Class<InodeMetadata>($I`InodeMetadata`)(
  ...
  $I.annote("InodeMetadata", {
    description: "Metadata for an inode"
  })
) {
}
...
class OpenFileDescriptor extends S.Class<OpenFileDescriptor>($I`OpenFileDescriptor`)(
  ...
  $I.annote("OpenFileDescriptor", {
    description: "An open file descriptor"
  })
) {
}
...
class State extends S.Class<State>($I`State`)(
  {
    ...
    nextInode: S.Finite,
    descriptors: S.HashMap(FileDescriptor, OpenFileDescriptor),
    nextDescriptor: S.Finite,
    nextTemporary: S.Finite,
  },
  $I.annote("State", {
    description: "The state of the memory file system"
  })
) {
}
...
class ResolveOptions extends S.Class<ResolveOptions>($I`ResolveOptions`)(
  {
    followFinalSymbolicLink: S.Boolean.pipe(SchemaUtils.withKeyDefaults(false)),
    method: S.String.pipe(
      SchemaUtils.withKeyDefaults("resolve")
    )
  },
  $I.annote("ResolveOptions", {
    description: "Options for resolving a path"
  })
) {
}
...
class ResolvedInode extends S.Class<ResolvedInode>($I`ResolvedInode`)(
  ...
  $I.annote("ResolvedInode", {
    description: "An inode that has been resolved"
  })
) {
}
...
class CloneContext extends S.Class<CloneContext>($I`CloneContext`)(
  ...
  $I.annote("CloneContext", {
    description: "Options for cloning a file"
  })
) {
}

const SystemErrorTag = LiteralKit(
  [
    "AlreadyExists",
    "BadResource",
    "Busy",
  ...
  $I.annoteSchema("SystemErrorTag", {
    description: "Tag for system errors"
  })
);
...
const findEntry = (directory: DirectoryInode, name: string): Inode | undefined =>
  O.getOrUndefined(HashMap.get(directory.entries, name));
...
const validateEntryName = Effect.fnUntraced(function* (
  ...
  const invalidName = Str.isEmpty(name) || name === "." || name === ".." || Str.includes("/")(name) || Str.includes("\0")(name);
  if (invalidName) {
...
const getDirectory = Effect.fnUntraced(function* (
  ...
  if (!InodeEntry.guards.Directory(entry)) {
...
const collectInodePaths = (
  ...
  for (const [name, childInode] of directory.entries) {
    ...
    if (InodeEntry.guards.Directory(child)) {
...
const inodeUpdateEvents = (
  ...
  if (!InodeEntry.guards.Directory(root)) return [];
...
const attachDirectory = Effect.fnUntraced(function* (
  ...
  const expectedUnlinkedLinks = entry._tag === "Directory"
    ? DIR_SELF_LINK_COUNT +
    [...HashMap.values(entry.entries)].filter((child) => InodeEntry.guards.Directory(findInode(state, child))).length
    : 0;
  if (!InodeEntry.guards.Directory(entry) || parent === inode || entry.nlink !== expectedUnlinkedLinks) {
...
const resolve = Effect.fnUntraced(function* (
  ...
  while (components.length > 0) {
    ...
    const shouldFollow = InodeEntry.guards.SymbolicLink(entry) &&
...
const resolveParent = Effect.fnUntraced(function* (
  ...
  const parent = yield* resolve(state, parentPath, ResolveOptions.make({method})).pipe(
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        ...
        for (let index = 0; index < pieces.length; index++) {
          ...
          const existing = yield* Effect.result(resolve(nextState, candidate, ResolveOptions.make({method})));
...
const link = (volume: Volume) =>
  Effect.fnUntraced(function* (fromPath: string, toPath: string) {
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        const source = yield* resolve(state, fromPath, ResolveOptions.make({method})).pipe(
...
const realPath = (volume: Volume) =>
  Effect.fnUntraced(function* (path: string) {
    return yield* volume.withState((state) => Effect.map(resolve(state, path, ResolveOptions.make({method: "realPath"})), (_) => _.path));
  });
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        ...
        if (target._tag === "Failure") {
          if (P.isNotUndefined(options?.force) && target.failure.reason._tag === "NotFound") {
...
const cloneInode: (
  ...
  function* (state, source, context) {
    ...
    return yield* InodeEntry.match(source, {
      File:
        Effect.fn("MemoryFileSystem.File")(function* (source) {
      ...
      SymbolicLink:
        Effect.fn("MemoryFileSystem.SymbolicLink")(function* (source) {
      ...
      Directory:
        Effect.fn("MemoryFileSystem")(function* (source) {
...
const resolveCopyFileDestination = Effect.fnUntraced(function* (
  ...
  while (true) {
    ...
    const resolved = yield* Effect.result(resolve(state, candidate, ResolveOptions.make({method})));
...
const copyFile = (volume: Volume) =>
  Effect.fnUntraced(
    function* (fromPath: string, toPath: string) {
      return yield* volume.mutate((state) =>
        Effect.gen(function* () {
          const existing = yield* Effect.result(resolve(state, toPath, ResolveOptions.make({method: "copyFile"})));
          ...
          const destination = yield* resolve(nextState, toPath, ResolveOptions.make({method: "copyFile"}));
          return transitionResult(
            nextState,
            undefined,
  ...
  function* (state, path, options) {
    ...
    while (entry === undefined) {
      ...
      if (unresolved._tag === "Success") {
        ...
        const resolved = yield* Effect.result(resolve(nextState, candidatePath, ResolveOptions.make({method: "open"})));
...
const openDescriptor: (
  ...
  function* (volume, path, options) {
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        const existing = yield* Effect.result(resolve(state, path, ResolveOptions.make({method: "open"})));
        ...
        const resolved = yield* resolve(nextState, path, ResolveOptions.make({method: "open"}));
        return transitionResult(
          nextState,
          descriptor.fd,
          tag === "Update"
...
class MemoryFile implements FileSystem.File {
  ...
  readAlloc(size: FileSystem.SizeInput): Effect.Effect<O.Option<Uint8Array>, PlatformError> {
        ...
        Effect.map(
          ...
          (bytesRead) =>
            bytesRead === FileSystem.Size(0)
              ? O.none()
              : O.some(bytesRead === FileSystem.Size(length) ? buffer : buffer.slice(0, Number(bytesRead)))
        )
    );
  }
...
const access = (volume: Volume) =>
  Effect.fnUntraced(function* (path: string) {
    yield* volume.withState((state) => Effect.asVoid(resolve(state, path, ResolveOptions.make({method: "access"}))));
  });
...
const readDirectory = (volume: Volume) =>
  Effect.fnUntraced(function* (
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        const resolved = yield* resolve(state, path, ResolveOptions.make({method: "readDirectory"}));
...
const readFile = (volume: Volume) =>
  Effect.fnUntraced(function* (path: string) {
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        const resolved = yield* resolve(state, path, ResolveOptions.make({method: "readFile"}));
...
const writeFile = (volume: Volume) =>
  (
    ...
    volume.mutate((state) =>
      Effect.gen(function* () {
        const existing = yield* Effect.result(resolve(state, path, ResolveOptions.make({method: "writeFile"})));
        ...
        const resolved = yield* resolve(nextState, path, ResolveOptions.make({method: "writeFile"}));
        return transitionResult(
          nextState,
          undefined,
          tag === "Update"
...
const truncate = (volume: Volume) =>
  Effect.fnUntraced(function* (
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        const resolved = yield* resolve(state, path, ResolveOptions.make({method: "truncate"}));
        if (resolved.entry._tag !== "File") {
...
const stat = (volume: Volume) =>
  Effect.fnUntraced(function* (path: string) {
    return yield* volume.withState((state) =>
      Effect.map(resolve(state, path, ResolveOptions.make({method: "stat"})), ({entry}) => fileInfo(entry))
    );
  });
...
const chmod = (volume: Volume) =>
  Effect.fnUntraced(function* (path: string, mode: number) {
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        const resolved = yield* resolve(state, path, ResolveOptions.make({method: "chmod"}));
        const now = yield* DateTime.now;
        const nextState = setInode(state, {
          ...resolved.entry,
...
const chown = (volume: Volume) =>
  Effect.fnUntraced(function* (
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        const resolved = yield* resolve(state, path, ResolveOptions.make({method: "chown"}));
        const now = yield* DateTime.now;
        const nextState = setInode(state, {
          ...resolved.entry,
          uid,
...
const utimes = (volume: Volume) =>
  Effect.fnUntraced(function* (
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        const resolved = yield* resolve(state, path, ResolveOptions.make({method: "utimes"}));
        const now = yield* DateTime.now;
        const nextState = setInode(state, {
          ...resolved.entry,
          atime: accessTime,
...
const allocateTempDirectory = Effect.fnUntraced(function* (
  ...
  const parent = yield* resolve(state, parentPath, ResolveOptions.make({method}));
  if (parent.entry._tag !== "Directory") {
...
const makeTempDirectoryScoped = (volume: Volume) =>
  (
    ...
    Effect.acquireRelease(
      ...
      (path) => Effect.orDie(remove(volume)(path, {
        recursive: true,
        force: true
      }))
    );
...
const matchesGlobToken = (token: GlobToken, value: string): boolean =>
  GlobToken.$match(token, {
    ...
    CharacterClass: (token) => {
      const matches = A.contains(token.literals, value) ||
...
const glob = (volume: Volume) =>
  Effect.fnUntraced(function* (
    ...
    return yield* volume.withState((state) =>
      Effect.gen(function* () {
        const resolved = yield* resolve(state, rootPath, ResolveOptions.make({method: "glob"}));
        if (resolved.entry._tag !== "Directory") {
      ...
      Effect.acquireRelease(
        volume.withState(
          Effect.fnUntraced(function* (state) {
            const resolved = yield* resolve(state, path, ResolveOptions.make({method: "stat"}));
