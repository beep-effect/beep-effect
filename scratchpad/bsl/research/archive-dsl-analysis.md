...
|---|---|---|
| `toDrizzle` | Re-export from `./adapters/drizzle`. | Forward conversion from a DSL model to a PostgreSQL Drizzle table. |
| `DSL` | `export * as DSL from "./combinators"`. | Namespace containing the pipe combinators. |
| `deriveColumnType`, `deriveSchemaColumnType` | Re-export from `./derive-column-type`. | Runtime AST-to-column-type derivation and its schema wrapper. |
...
This is the full exported surface of `packages/common/schema/src/integrations/sql/dsl/types.ts`.
