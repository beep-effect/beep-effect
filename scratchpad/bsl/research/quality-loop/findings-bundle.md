...
- `suggestedFix`: Add dialect-local kit implementations and exports to the existing `./pg` and `./sqlite` entrypoints, while retaining the root dispatcher as an explicitly documented convenience API. Bundle-sensitive consumers must be able to import a kit constructor whose source graph never reaches the sibling dialect.
...
- `impact`: A selective `./pg` or `./sqlite` import can retain unrelated descriptor constructors and their closures even after unused exports are removed. This weakens the intended benefit of named imports and the future `sideEffects: false` declaration.
