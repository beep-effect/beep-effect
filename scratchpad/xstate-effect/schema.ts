 ...
 * Schema, so XState infers the decoded `S.Type` without an explicit call
 * to `S.toStandardSchemaV1`.
 */
export type EffectSchema = S.ConstraintDecoder<unknown>;

/**
 * Either an Effect {@link EffectSchema} or a Standard S. Every schema
...
type AssertNonTransformingSchema<TSchema> = TSchema extends EffectSchemaLike
  ? IsAny<StandardSchemaV1.InferInput<ToStandardSchema<TSchema>>> extends true
    ? TSchema
    : IsAny<StandardSchemaV1.InferOutput<ToStandardSchema<TSchema>>> extends true
      ? TSchema
      : [
...
function isStandardSchema(value: unknown): value is StandardSchemaV1 {
  return P.hasProperty(value, "~standard");
}

export function toStandardSchema(schema: EffectSchemaLike): StandardSchemaV1 {
  if (S.isSchema(schema)) {
    return S.toStandardSchemaV1(schema as EffectSchema);
  }
  return schema as StandardSchemaV1;
}

function mapSchemaRecord(value: Record<string, unknown>): Record<string, unknown> {
  return R.fromEntries(
    A.map(R.keys(value), (key) => {
      const entry = value[key];
      if (S.isSchema(entry) || isStandardSchema(entry)) {
...
export function toStandardSetupSchemas(schemas: EffectSetupSchemas | undefined): SetupSchemas | undefined {
  return schemas === undefined ? undefined : mapSchemaRecord(schemas);
}
      ...
      return [
        key,
        {
          ...
          ...(state.schemas !== undefined
            ...
            : undefined),
          ...(state.states !== undefined ? { states: toStandardSetupStates(state.states) } : undefined),
        },
      ];
    })
