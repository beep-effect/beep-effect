...
import { createEffectActor, type EffectActorOptions } from "./createEffectActor.ts";
import type { EffectActor } from "./effectActor.ts";
import { NotReadyError } from "./errors.ts";
import type { ErrorFrom, RequirementsFrom } from "./types.ts";
 ...
 * actor per input.
 */
export const createActorAtoms: {
  <TLogic extends AnyActorLogic, ER = never>(
    logic: TLogic,
    options?: EffectActorOptions<TLogic>
  ): (runtime: Atom.AtomRuntime<RequirementsFrom<TLogic>, ER>) => ActorAtoms<TLogic, ER>;
  <TLogic extends AnyActorLogic, ER = never>(
    runtime: Atom.AtomRuntime<RequirementsFrom<NoInfer<TLogic>>, ER>,
    logic: TLogic,
    options?: EffectActorOptions<TLogic>
  ...
  <TLogic extends AnyActorLogic, ER = never>(
    runtime: Atom.AtomRuntime<RequirementsFrom<TLogic>, ER>,
    logic: TLogic,
    options?: EffectActorOptions<TLogic>
  ): ActorAtoms<TLogic, ER> => {
    const actor = runtime.atom(createEffectActor(logic, options as never));

    const snapshot = Atom.make((get): AsyncResult.AsyncResult<SnapshotFrom<TLogic>, ER> => {
    ...
    const result = Atom.make((get): AsyncResult.AsyncResult<SnapshotFrom<TLogic>, ER | ErrorFrom<TLogic>> => {
      ...
      if (value.status === "error") {
        return AsyncResult.failureWithPrevious(Cause.fail(value.error as ErrorFrom<TLogic>), { previous: get.self() });
      }
      return current;
    });

    const sendAtom = Atom.writable<AsyncResult.AsyncResult<void, ER | NotReadyError>, EventFromLogic<TLogic>>(
    ...
    function select<T>(selector: (snapshot: SnapshotFrom<TLogic>) => T): Atom.Atom<AsyncResult.AsyncResult<T, ER>> {
