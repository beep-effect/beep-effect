...
import type { ErrorFrom } from "./types.ts";
...
export type EmittedEventFrom<TActor> =
  TActor extends EffectActor<infer TLogic>
    ...
    : TActor extends Actor<infer TLogic>
      ? EmittedFrom<TLogic>
      : TActor extends ActorRef<infer _Snapshot, infer _Event, infer TEmitted, infer _Send>
        ? TEmitted
        : AnyEventObject;
...
function isActorRef(value: unknown): value is AnyActorRef {
  return (
    P.hasProperty(value, "send") &&
    P.isFunction(value.send) &&
    P.hasProperty(value, "getSnapshot") &&
    P.isFunction(value.getSnapshot)
  );
}
 ...
 */
export function emitted<TActor extends AnyActorRef>(actor: TActor): Stream.Stream<EmittedEventFrom<TActor>> {
...
export const waitFor: {
  <TActor extends AnyActorRef, TNarrowed extends SnapshotFrom<TActor>>(
    predicate: P.Refinement<SnapshotFrom<TActor>, TNarrowed>
  ...
  ): (actor: TActor) => Effect.Effect<TNarrowed, ActorStoppedError | Cause.TimeoutError>;
  <TActor extends AnyActorRef>(
    predicate: (snapshot: SnapshotFrom<TActor>) => boolean,
    options: WaitForOptions
  ): (actor: TActor) => Effect.Effect<SnapshotFrom<TActor>, ActorStoppedError | Cause.TimeoutError>;
  <TActor extends AnyActorRef, TNarrowed extends SnapshotFrom<TActor>>(
    actor: TActor,
    predicate: P.Refinement<SnapshotFrom<TActor>, TNarrowed>
  ...
  <TActor extends AnyActorRef>(
    ...
    const waiting = Effect.callback<SnapshotFrom<TActor>, ActorStoppedError>((resume) => {
...
export function join<TActor extends AnyActorRef>(
  ...
  return Effect.callback<OutputFrom<TActor>, ErrorFrom<TActor> | ActorStoppedError>((resume) => {
    const settle = () => {
...
export function inspect(actor: AnyActorRef): Stream.Stream<InspectionEvent> {
  return Stream.callback<InspectionEvent>((queue) =>
    Effect.acquireRelease(
      Effect.sync(() => {
        const observer: (inspectionEvent: InspectionEvent) => void = (inspectionEvent) => {
          Queue.offerUnsafe(queue, inspectionEvent);
        };
        const inspectable = actor as {
          inspect?: (observer: (inspectionEvent: InspectionEvent) => void) => Subscription;
        };
 ...
 */
export function deadLetters(actor: AnyActorRef): Stream.Stream<DeadLetterInspectionEvent> {
  return Stream.filter(
    inspect(actor),
    (event): event is DeadLetterInspectionEvent => event.type === "@xstate.deadletter"
  );
}
