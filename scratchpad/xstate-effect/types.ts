...
export type EffectRequirements<T> = T extends Effect.Effect<unknown, unknown, infer R> ? R : never;
  ...
  T extends Record<string, (...args: never[]) => unknown>
    ? T[keyof T] extends infer TAction
      ? TAction extends {
        ...
        ? unknown extends R
          ? EffectRequirements<TAction extends (...args: never[]) => infer TResult ? TResult : never>
          : Exclude<R, undefined>
        : EffectRequirements<TAction extends (...args: never[]) => infer TResult ? TResult : never>
      : never
    : never;
...
type RequirementsFromStateSchema<TSchema, TDepth extends Depth> =
  | (TSchema extends { invoke: infer TInvoke } ? RequirementsFromInvoke<TInvoke, TDepth> : never)
...
export type RequirementsFrom<T, TDepth extends Depth = MaxDepth> =
  T extends EffectLogicBrand<unknown, infer R>
    ? R
    : T extends {
      ...
      ?
          ...
          | ([TDepth] extends [0]
              ...
              :
                  ...
                  | RequirementsFromStateSchema<TStateSchema, PrevDepth[TDepth]>)
      : never;

/** The error carried by the error snapshot of an actor or actor logic. */
export type ErrorFrom<T> = Exclude<Extract<SnapshotFrom<T>, { status: "error" }>["error"], undefined>;
