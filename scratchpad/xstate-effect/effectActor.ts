...
export const actionFailure: unique symbol = Symbol.for("@xstate/effect/actionFailure");

export function isActionFailure(value: unknown): value is ActionFailure {
  return P.hasProperty(value, actionFailure) && value[actionFailure] === true;
}

export type MailboxItem<TEvent> = TEvent | ActionFailure;

const symbolObservable: typeof Symbol.observable = Symbol.observable;
...
function toObserver<T>(
  ...
  if (P.isObject(nextHandler)) {
    return nextHandler;
  }
  return getSomesStruct({
    next: O.fromUndefinedOr(nextHandler),
    error: O.fromUndefinedOr(errorHandler),
    complete: O.fromUndefinedOr(completionHandler),
  });
}
 ...
 */
export class EffectActor<TLogic extends AnyActorLogic>
  implements ActorRef<SnapshotFrom<TLogic>, EventFromLogic<TLogic>, EmittedFrom<TLogic>>
{
  ...
  private readonly _listeners = new Map<string, Set<(emitted: EmittedFrom<TLogic>) => void>>();
  ...
  constructor(
    ...
    _mailbox: Queue.Queue<MailboxItem<EventFromLogic<TLogic>>>,
    _stopExecution: () => void,
    _inspectors: Set<(event: InspectionEvent) => void>
  ) {
    this.logic = logic;
  ...
  subscribe(
    nextListenerOrObserver?: ((snapshot: SnapshotFrom<TLogic>) => void) | Observer<SnapshotFrom<TLogic>>,
    errorListener?: (error: unknown) => void,
    completeListener?: () => void
  ): Subscription {
  ...
  on<TType extends EmittedFrom<TLogic>["type"] | "*">(
    type: TType,
    handler: (emitted: EmittedFrom<TLogic> & (TType extends "*" ? unknown : { type: TType })) => void
  ): Subscription {
  ...
  inspect(observer: Observer<InspectionEvent> | ((event: InspectionEvent) => void)): Subscription {
    const handler = P.isFunction(observer) ? observer : (event: InspectionEvent) => observer.next?.(event);
